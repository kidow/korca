...
Issue https://github.com/stablyai/korca-internal/issues/101 reports that the GitHub issues list can stay stale after creating a new issue.
...
The post-create flow forces only Korca's renderer-side work-item cache. It does not bypass the GitHub CLI REST cache used by the main-process recent work-item fetch, so the refreshed request can reuse stale `gh api --cache 120s` data.
...
1. Add an optional `noCache` flag to the renderer work-item fetch options and the GitHub work-items list contract:
   ...
   - `KorcaRuntime.listRepoWorkItems`;
...
- External GitHub mutations: external issue changes still rely on existing TTL/manual refresh behavior; this fix only guarantees freshness for Korca-originated create flows.
...
- Unit: extend `src/main/runtime/rpc/methods/github.test.ts` and/or `src/main/runtime/korca-runtime.test.ts` for `noCache` schema/forwarding.
