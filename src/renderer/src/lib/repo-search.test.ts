...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'stablyai/korca', path: '/repos/korca' }),
      makeRepo({ id: '2', displayName: 'stablyai/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'KORCA').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/korca' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/korca-tools/misc' }),
      makeRepo({ id: '2', displayName: 'korca', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'korca').map((repo) => repo.id)).toEqual(['2', '1'])
  })
})
