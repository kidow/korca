...
describe('getSmartGitHubSubmitIntent', () => {
  it('treats GitHub issue and pull URLs as submit-time source intent', () => {
    expect(getSmartGitHubSubmitIntent('https://github.com/stablyai/korca/pull/2049')).toEqual({
      kind: 'link',
      owner: 'stablyai',
      repo: 'korca',
      number: 2049,
    ...
    expect(getSmartGitHubSubmitIntent('https://github.com/stablyai/korca/issues/2050')).toEqual({
      kind: 'link',
      owner: 'stablyai',
      repo: 'korca',
      number: 2050,
...
describe('lookupSmartGitHubSubmitItem', () => {
  ...
  it('reuses an in-flight direct URL lookup for the same repo and intent', async () => {
    const item = {
      ...
      state: 'open' as const,
      url: 'https://github.com/stablyai/korca/pull/2049',
      labels: [],
  ...
  it('prunes expired distinct lookup entries on later lookups', async () => {
    ...
    const workItem = vi.fn().mockImplementation(({ number }) =>
      Promise.resolve({
        ...
        title: `Issue ${number}`,
        state: 'open' as const,
        url: `https://github.com/stablyai/korca/issues/${number}`,
        labels: [],
        updatedAt: '2026-05-26T00:00:00.000Z',
        author: 'octocat',
        repoId: 'repo-1'
...
describe('getSmartGitHubSubmitResolution', () => {
  ...
  it('uses the resolved item title for workspace name and linked issue metadata', () => {
    const resolution = getSmartGitHubSubmitResolution({
      ...
      url: 'https://github.com/stablyai/korca/issues/2050'
    })
