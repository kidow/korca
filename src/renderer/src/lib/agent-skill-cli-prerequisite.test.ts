...
describe('isKorcaCliAvailableOnPath', () => {
  it('requires the installed CLI command to be visible on PATH', () => {
    expect(isKorcaCliAvailableOnPath(cliStatus())).toBe(true)
    expect(isKorcaCliAvailableOnPath(cliStatus({ pathConfigured: false }))).toBe(false)
    expect(isKorcaCliAvailableOnPath(cliStatus({ state: 'not_installed' }))).toBe(false)
  })
})

describe('ensureKorcaCliAvailableForAgentSkillTerminal', () => {
  afterEach(() => {
    vi.useRealTimers()
    vi.unstubAllGlobals()
    vi.clearAllMocks()
  ...
  it('lets the registration toast paint before opening the native installer', async () => {
    ...
    const pending = ensureKorcaCliAvailableForAgentSkillTerminal({ registrationPromptDelayMs: 700 })
    await vi.waitFor(() => {
      expect(toast.message).toHaveBeenCalledWith(CLI_PREREQUISITE_REGISTRATION_TOAST, {
        description: CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION
      })
