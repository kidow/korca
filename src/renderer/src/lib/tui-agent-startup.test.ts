...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses KORCA_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Korca overlay installs an
    // `korca-prefill` extension that reads KORCA_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset KORCA_PI_PREFILL',
      expectedProcess: 'pi',
      env: { KORCA_PI_PREFILL: 'https://github.com/acme/repo/issues/42' }
    })
  })
