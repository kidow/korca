...
import { KORCA_EDITOR_REQUEST_CMD_SAVE_EVENT } from './editor-autosave'
...
export function useEditorCmdSaveRequest({
  ...
  useEffect(() => {
    ...
    window.addEventListener(KORCA_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
    return () => window.removeEventListener(KORCA_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
