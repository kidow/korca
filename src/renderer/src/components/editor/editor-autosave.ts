...
export const KORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT = 'korca:editor-quiesce-file-saves'
export const KORCA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'korca:editor-external-file-change'
export const KORCA_EDITOR_SAVE_FILE_EVENT = 'korca:editor-save-file'
export const KORCA_EDITOR_SAVE_AND_CLOSE_EVENT = 'korca:save-and-close'
export const KORCA_EDITOR_FILE_SAVED_EVENT = 'korca:editor-file-saved'
export const KORCA_EDITOR_REQUEST_CMD_SAVE_EVENT = 'korca:editor-request-cmd-save'
export const KORCA_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'korca:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(KORCA_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(KORCA_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(KORCA_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(KORCA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
