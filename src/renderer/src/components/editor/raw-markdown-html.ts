...
const INLINE_PLACEHOLDER_PREFIX = '[[KORCA_RAW_HTML_INLINE:'
const BLOCK_PLACEHOLDER_PREFIX = '[[KORCA_RAW_HTML_BLOCK:'
...
export function encodeRawMarkdownHtmlForRichEditor(content: string): string {
  ...
  while (index < content.length) {
    ...
    // skipped by the guards above. The [[KORCA_ prefix check prevents re-encoding
    // sibling placeholders that were already emitted earlier in this pass.
    if (
      content[index] === '[' &&
      content[index + 1] === '[' &&
      !content.startsWith('[[KORCA_', index) &&
...
export const RawMarkdownHtmlInline = Node.create({
  ...
  // while still letting Korca round-trip the raw markup verbatim.
