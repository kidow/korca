...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for KORCA_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'korca_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps korca_disabled to KORCA_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('korca_disabled')).toBe('KORCA_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names KORCA_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'korca_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'korca_disabled' })
  })
