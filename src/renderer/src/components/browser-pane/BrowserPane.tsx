...
import { KORCA_BROWSER_BLANK_URL, KORCA_BROWSER_PARTITION } from '../../../../shared/constants'
import type {
  BrowserLoadError,
...
function toDisplayUrl(url: string): string {
  return url === KORCA_BROWSER_BLANK_URL ? 'about:blank' : redactKagiSessionToken(url)
}
...
function evictParkedWebviews(excludedTabId: string | null = null): void {
  ...
  while (webviewRegistry.size > MAX_PARKED_WEBVIEWS && parkedBrowserTabIds.length > 0) {
    ...
    if (browserTabId) {
      ...
      // cannot grow without bound or long Korca sessions accumulate Chromium
  ...
  const ensureRemotePage = useCallback(
    async (token: RemoteBrowserOperationToken): Promise<string | null> => {
      ...
      const createRemotePage = async (): Promise<string | null> => {
        ...
        const initialUrl =
          currentUrl === KORCA_BROWSER_BLANK_URL ? 'about:blank' : currentUrl || 'about:blank'
...
function BrowserPagePane({
  ...
  const sessionProfile = sessionProfileId
    ...
    : null
  const webviewPartition = sessionProfile?.partition ?? KORCA_BROWSER_PARTITION
  ...
  useEffect(() => {
    return window.api.browser.onDownloadRequested((event) => {
      ...
      // triggered the save prompt before Korca asks main to choose a path.
      setDownloadState({
        ...event,
        receivedBytes: 0,
        status: 'requested'
  ...
  useEffect(() => {
    ...
    window.addEventListener(KORCA_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () =>
      window.removeEventListener(KORCA_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
  }, [browserTab.id, focusAddressBarNow, focusWebviewNow, isActive])
    ...
    (webview: Electron.WebviewTag): void => {
      try {
        onUpdatePageStateRef.current(browserTab.id, {
          ...
          // Only explicit navigation/load events should drive Korca's loading UI.
          canGoBack: webview.canGoBack(),
          canGoForward: webview.canGoForward()
        })
      } catch {
        // Why: Electron only exposes these getters after the guest fully
        // attaches. Ignoring the transient failure avoids crashing Korca while
  ...
  useEffect(() => {
    ...
    if (webview) {
      ...
      // (e.g. httpbin.org/html) don't show through to Korca's dark chrome. Real
    ...
    const handleDidStopLoading = (): void => {
      ...
      if (activeLoadFailure) {
        ...
        if (normalizedAttemptedUrl === normalizedCurrentUrl) {
          ...
          // navigation falls back to a blank Chromium surface even though Korca
      ...
      if (keepAddressBarFocusRef.current && currentUrl === KORCA_BROWSER_BLANK_URL) {
        focusAddressBarNow()
      } else {
        keepAddressBarFocusRef.current = false
      }
    ...
    const handleFailLoad = (event: {
      ...
      if (event.errorCode === -3) {
        ...
        // when the replacement navigation succeeds. Ignore that noise so Korca
    ...
    if (needsInitialNavigation) {
      ...
      // Korca never misses the initial did-fail-load signal for a new tab.
      // Only non-blank initial tabs should light up Korca's loading indicator;
      // reclaiming/activating a parked about:blank tab is not a meaningful
      // navigation and should not flash the tab-loading dot.
      const initialUrl =
        normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? KORCA_BROWSER_BLANK_URL
      trackNextLoadingEventRef.current = initialUrl !== KORCA_BROWSER_BLANK_URL
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Why: browserTab.url changes are Korca-driven navigations (address bar,
      ...
      trackNextLoadingEventRef.current = normalizedUrl !== KORCA_BROWSER_BLANK_URL
      lastKnownWebviewUrlRef.current = normalizedUrl
      webview.src = normalizedUrl
      if (normalizedUrl !== KORCA_BROWSER_BLANK_URL) {
        keepAddressBarFocusRef.current = false
        if (document.activeElement === addressBarInputRef.current) {
          focusWebviewNow()
        }
  ...
  useEffect(() => {
    ...
    // Polling only while the active tab is "loading" gives Korca a last-resort
  ...
  const navigateToUrl = useCallback(
    (url: string): void => {
      const navigateBrowserUrl = (targetUrl: string): void => {
        ...
        trackNextLoadingEventRef.current = targetUrl !== KORCA_BROWSER_BLANK_URL
        lastKnownWebviewUrlRef.current =
          normalizeBrowserNavigationUrl(browserModelUrl) ?? browserModelUrl
        webview.src = targetUrl
        if (targetUrl !== KORCA_BROWSER_BLANK_URL) {
          focusWebviewNow()
        }
      }
