...
function makeFile(overrides: Partial<OpenFile> = {}): OpenFile {
  ...
  return {
    id,
    filePath: overrides.filePath ?? `/tmp/korca/${id}.md`,
...
describe('FloatingTerminalPanel close behavior', () => {
  beforeEach(() => {
    ...
    mocks.getFloatingMarkdownDirectory.mockResolvedValue('/tmp/korca/floating-notes')
    mocks.getFloatingTerminalCwd.mockResolvedValue('/tmp/korca')
  ...
  it('creates floating markdown files in local filesystem mode', async () => {
    ...
    vi.mocked(createUntitledMarkdownFileWithTemplateSelection).mockResolvedValue({
      filePath: '/tmp/korca/floating-notes/untitled.md',
    ...
    expect(createUntitledMarkdownFileWithTemplateSelection).toHaveBeenCalledWith(
      '/tmp/korca/floating-notes',
      FLOATING_TERMINAL_WORKTREE_ID,
      undefined,
      { activeRuntimeEnvironmentId: null }
    )
    expect(mocks.openFile).toHaveBeenCalledWith(
      expect.objectContaining({ filePath: '/tmp/korca/floating-notes/untitled.md' }),
