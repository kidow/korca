...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'korca::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'korca',
      repoName: 'KORCA',
      cpu: 1.5,
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      { id: 'korca::/remote/Stingray@@abcd1234', cwd: '', title: 'korca/Stingray' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['korca', 'ssh-conn-1']])
    })
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'korca::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'korca::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      { id: 'korca::/local/Triton@@deadbeef', cwd: '/local/Triton', title: 'korca/Triton' }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['korca', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'korca',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    const tabId = 'tab-xyz'
    const ds: DaemonSession[] = [{ id: 'korca::/wrong/path@@feedface', cwd: '', title: 'korca' }]
    const ctx = baseCtx({
      tabsByWorktree: {
        'korca::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['korca::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('korca::/correct/path')
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'korca::/Users/me/Triton': [makeTab(tabId)] },
  ...
  it('remote-orphan interaction state: null metrics + bound=false', () => {
    const ds: DaemonSession[] = [{ id: 'korca::/remote/Wt@@deadbeef', cwd: '', title: 'korca/Wt' }]
    const out = mergeSnapshotAndSessions(null, ds, baseCtx())
    const session = out[0].worktrees[0].sessions[0]
    expect(session).toMatchObject({
      hasLocalSamples: false,
  ...
  it('uses repoDisplayNameById to humanize new project groups when available', () => {
    const ds: DaemonSession[] = [{ id: 'stably-ai/korca::/remote/Wt@@1', cwd: '', title: '' }]
    const ctx = baseCtx({
      repoDisplayNameById: new Map([['stably-ai/korca', 'KORCA']])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].repoName).toBe('KORCA')
  })
