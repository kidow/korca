...
import { showKorcaCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
import {
...
const FEATURE_SKILL_NAMES: Record<OnboardingFeatureSetupId, string> = {
  browserUse: KORCA_CLI_SKILL_NAME,
...
export function createOnboardingFeatureSetupDeps(): OnboardingFeatureSetupDeps {
  ...
  return {
    ...
    showCliRegistrationPrompt: showKorcaCliRegistrationPromptToast,
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Korca CLI registration is not available on this platform.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Korca CLI registration needs attention.'
        })
