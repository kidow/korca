...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  const [yamlHooks, setYamlHooks] = useState<KorcaHooks | null>(null)
  ...
  const commitHookCheckIfCurrent = useCallback(
    (targetRepoId: string, hooks: KorcaHooks | null): boolean => {
  ...
  useEffect(() => {
    ...
    // number and still get a concrete selectable result. Korca mirrors that by
  ...
  const uploadComposerPaths = useCallback(
    async (
      ...
      const destinationDir = joinPath(targetRepoPath, '.korca/drops')
  ...
  const handleBaseBranchPrSelect = useCallback(
    (
      ...
      // worktree's comment should surface (`korca worktree current`, sidebar).
