...
export type TuiAgentConfig = {
   ...
   * submitting. Today only pi uses this (via Korca's overlay-installed
   * `korca-prefill` extension reading `KORCA_PI_PREFILL`). Equivalent in
   ...
   * `›` prompt only when the composer row exists, so Korca can paste as soon
...
// knowledge staying in sync: how Korca detects the agent on PATH, which binary
...
export const TUI_AGENT_CONFIG: Record<TuiAgent, TuiAgentConfig> = {
  claude: {
    ...
    // See PR https://github.com/stablyai/korca/pull/926 for context.
  ...
  codex: {
    ...
    promptInjectionMode: 'argv',
    // Why: Codex's positional prompt auto-submits the first turn, so Korca
  ...
  pi: {
    ...
    // extensions list) so the paste frequently never lands. Korca's
    // overlay installs an `korca-prefill` pi extension (see
    ...
    draftPromptEnvVar: 'KORCA_PI_PREFILL'
  },
  omp: {
    ...
    // Pi's command-line parser, but every Korca-owned env var (overlay
    // shadow, prefill) is scoped to OMP - see KORCA_OMP_* in
    ...
    expectedProcess: 'omp',
    promptInjectionMode: 'argv',
    draftPromptEnvVar: 'KORCA_OMP_PREFILL'
  },
  gemini: {
    detectCmd: 'gemini',
    launchCmd: 'gemini',
  ...
  'command-code': {
    ...
    // mirrors the preflight trust behavior Korca applies to other first-run
  ...
  hermes: {
    ...
    // `--tui` starts the full-screen agent UI Korca is designed to host.
  ...
  copilot: {
    ...
    // completion, which would kill the TUI session Korca is hosting.
    // `-i/--interactive <prompt>` starts an interactive session with the
    // initial prompt pre-executed — the behavior Korca needs.
