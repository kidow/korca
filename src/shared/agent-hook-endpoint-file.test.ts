...
describe('agent hook endpoint files', () => {
    ...
    expect(
      parseAgentHookEndpointFile(
        [
          ...
          'KORCA_AGENT_HOOK_TOKEN=token-123',
  ...
  it('parses Windows endpoint.cmd contents', () => {
    expect(
      parseAgentHookEndpointFile(
        [
          'set KORCA_AGENT_HOOK_PORT=54321',
          'set KORCA_AGENT_HOOK_TOKEN=token-abc',
          'set KORCA_AGENT_HOOK_ENV=development',
          'set KORCA_AGENT_HOOK_VERSION=1'
        ].join('\r\n')
      )
    ).toEqual({
      port: '54321',
  ...
  it('preserves equals signs in endpoint values', () => {
    expect(
      parseAgentHookEndpointFile(
        [
          'KORCA_AGENT_HOOK_PORT=12345',
          'KORCA_AGENT_HOOK_TOKEN=token=with=equals',
          'KORCA_AGENT_HOOK_ENV=production',
          'KORCA_AGENT_HOOK_VERSION=1'
        ].join('\n')
      ).token
  ...
  it('throws when required endpoint fields are missing', () => {
    expect(() => parseAgentHookEndpointFile('KORCA_AGENT_HOOK_PORT=12345')).toThrow(
