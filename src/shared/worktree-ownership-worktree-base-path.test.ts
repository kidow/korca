...
import { buildKnownKorcaWorkspaceLayouts, classifyWorktreeOwnership } from './worktree-ownership'
...
describe('repo-specific worktree ownership layouts', () => {
  it('resolves the same relative base path from each repo root', () => {
    ...
    expect(buildKnownKorcaWorkspaceLayouts(settings, repoA)[0]).toEqual({
      path: '/projects/a/worktrees',
      nestWorkspaces: true
    })
    expect(buildKnownKorcaWorkspaceLayouts(settings, repoB)[0]).toEqual({
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        knownKorcaLayouts: buildKnownKorcaWorkspaceLayouts(settings, repoA)
      })
    ).toBe('korca-managed')
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree('/projects/a/worktrees/repo/feature'),
        knownKorcaLayouts: buildKnownKorcaWorkspaceLayouts(settings, repoB)
      })
    ).toBe('external')
  })
  ...
  it('includes relative global layouts for SSH repos without applying absolute desktop paths', () => {
    ...
    expect(buildKnownKorcaWorkspaceLayouts(relativeSettings, repo)[0]).toEqual({
      path: '/remote/worktrees',
      nestWorkspaces: true
    })
    expect(
      buildKnownKorcaWorkspaceLayouts(absoluteSettings, repo).some(
