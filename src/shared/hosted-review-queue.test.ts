...
function baseSummary(overrides: Partial<HostedReviewQueueSummary> = {}): HostedReviewQueueSummary {
  return {
    identity: { provider: 'github', host: 'github.com', owner: 'acme', repo: 'korca', number: 42 },
...
describe('classifyHostedReview', () => {
  it('classifies mine/requested/agent/teammate', () => {
    ...
    expect(
      classifyHostedReview(baseSummary({ author: { login: 'korca-ci' } }), {
        agentAuthorLogins: ['korca-ci']
      }).state
    ).toBe('agent')
