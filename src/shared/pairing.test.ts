...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^korca:\/\/pair\?code=/)
  ...
  it('rejects korca URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`korca://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`korca://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`korca://pairing?code=${code}`)).toThrow('Invalid pairing URL')
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('korca://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`korca://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`korca://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full korca://pair# URL', () => {
    const url = encodePairingOffer(offer)
