...
// the relay sends to Korca. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Korca's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Korca routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Korca's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Korca's
//   local handle on an `ssh2` connection, not a wire identity. Korca stamps the
...
//   POST body so Korca's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Korca can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Korca. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Korca's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Korca can
  ...
  /** True when the relay is replaying its cache after Korca reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Korca's warn-once
   ...
   *  Korca's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Korca issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Korca issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Korca and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const KORCA_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'KORCA_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[KORCA_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
