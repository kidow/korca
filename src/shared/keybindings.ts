...
export type TerminalShortcutPolicy = 'korca-first' | 'terminal-first'
...
export function normalizeTerminalShortcutPolicy(
  ...
  return policy === 'terminal-first' ? 'terminal-first' : 'korca-first'
}
...
export function keybindingIsActiveInContext(
  ...
  // Why: Korca-first preserves existing app shortcut behavior inside terminals.
  // Terminal-first is the explicit escape hatch for shells and TUIs.
  if (normalizeTerminalShortcutPolicy(options.terminalShortcutPolicy) === 'korca-first') {
    return true
  }
