...
import { launchKorcaApp } from './launch'
...
export class RuntimeClient {
  ...
  constructor(
    ...
    remotePairingCode = process.env.KORCA_PAIRING_CODE ?? process.env.KORCA_REMOTE_PAIRING ?? null,
    environmentSelector = process.env.KORCA_ENVIRONMENT ?? null
  ) {
    ...
    if (this.remotePairing) {
      ...
      return {
        ...
        result: {
          ...
          // that this client machine has a local Korca desktop process.
          app: {
            running: false,
            pid: null
          },
  ...
  async openKorca(timeoutMs = 15_000): Promise<RuntimeRpcSuccess<CliStatusResult>> {
    ...
    throw new RuntimeClientError(
      ...
      'Timed out waiting for Korca to start. Run the Korca app manually and try again.'
    )
  }
}
...
function resolveRemotePairing(
  ...
  if (!pairing) {
    throw new RuntimeClientError(
      'invalid_argument',
      'Invalid remote pairing code. Expected an korca://pair?... URL or bare pairing payload.'
    )
  }
  return pairing
}
