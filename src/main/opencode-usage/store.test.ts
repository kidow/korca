...
describe('OpenCodeUsageStore', () => {
  ...
  it('reports no data for Korca scope when only non-Korca OpenCode usage exists', async () => {
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('korca', '30d')
    const daily = await store.getDaily('korca', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('korca', '30d', 5)
