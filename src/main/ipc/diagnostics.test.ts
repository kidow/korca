...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { KORCA_BUILD_IDENTITY?: unknown }).KORCA_BUILD_IDENTITY
    delete (globalThis as { KORCA_DIAGNOSTICS_TOKEN_URL?: unknown }).KORCA_DIAGNOSTICS_TOKEN_URL
    process.env.KORCA_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      KORCA_BUILD_IDENTITY?: 'stable'
      KORCA_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.KORCA_BUILD_IDENTITY = 'stable'
    globalOverrides.KORCA_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.KORCA_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
