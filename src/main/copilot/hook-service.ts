...
// names, which match the event vocabulary already normalized by Korca's hook
...
const COPILOT_EVENTS = [
  ...
  // Korca can normalize it without depending on a PascalCase payload.
  'subagentStart',
  'SubagentStop',
  'PreCompact',
  'Stop',
...
function getConfigPath(): string {
  return join(getCopilotHome(), 'hooks', 'korca.json')
}
...
function getManagedCommand(scriptPath: string, eventName: string): string {
  return process.platform === 'win32'
    ? `$env:KORCA_COPILOT_HOOK_EVENT = '${eventName}'; powershell.exe -NoProfile -ExecutionPolicy Bypass -File ${quotePowerShellPath(scriptPath)}`
    : wrapPosixHookCommand(scriptPath, { KORCA_COPILOT_HOOK_EVENT: eventName })
}
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      // lines so surviving PTYs can refresh to the current Korca server.
      'if ($env:KORCA_AGENT_HOOK_ENDPOINT -and (Test-Path -LiteralPath $env:KORCA_AGENT_HOOK_ENDPOINT)) {',
      '  try {',
      '    Get-Content -LiteralPath $env:KORCA_AGENT_HOOK_ENDPOINT | ForEach-Object {',
      ...
      '  } catch {}',
      '}',
      'if (-not $env:KORCA_AGENT_HOOK_PORT -or -not $env:KORCA_AGENT_HOOK_TOKEN -or -not $env:KORCA_PANE_KEY) { exit 0 }',
      '$inputData = [Console]::In.ReadToEnd()',
      'if ([string]::IsNullOrWhiteSpace($inputData)) { exit 0 }',
      'try {',
      '  $payload = $inputData | ConvertFrom-Json',
      '  $body = @{',
      '    paneKey = $env:KORCA_PANE_KEY',
      '    tabId = $env:KORCA_TAB_ID',
      '    worktreeId = $env:KORCA_WORKTREE_ID',
      '    hookEventName = $env:KORCA_COPILOT_HOOK_EVENT',
      '    env = $env:KORCA_AGENT_HOOK_ENV',
      '    version = $env:KORCA_AGENT_HOOK_VERSION',
      '    payload = $payload',
      '  } | ConvertTo-Json -Depth 100',
      "  Invoke-WebRequest -UseBasicParsing -Method Post -Uri ('http://127.0.0.1:' + $env:KORCA_AGENT_HOOK_PORT + '/hook/copilot') -Headers @{ 'Content-Type'='application/json'; 'X-Korca-Agent-Hook-Token'=$env:KORCA_AGENT_HOOK_TOKEN } -Body $body -TimeoutSec 2 | Out-Null",
      '} catch {}',
      'exit 0',
      ''
    ].join('\r\n')
  ...
  return [
    ...
    '  exit 0',
    'fi',
    'curl -sS -X POST "http://127.0.0.1:${KORCA_AGENT_HOOK_PORT}/hook/copilot" \\',
    ...
    '  --data-urlencode "hookEventName=${KORCA_COPILOT_HOOK_EVENT}" \\',
...
export class CopilotHookService {
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    const remoteConfigPath = `${home}/.copilot/hooks/korca.json`
    const remoteScriptPath = `${home}/.korca/agent-hooks/copilot-hook.sh`
    ...
    try {
      ...
      if (!config) {
        return {
          ...
          managedHooksPresent: false,
          detail: 'Could not parse remote Copilot hooks/korca.json'
        }
      }
      ...
      for (const eventName of COPILOT_EVENTS) {
        ...
        nextHooks[eventName] = [
          ...
          getRemoteManagedHookDefinition(
            wrapPosixHookCommand(remoteScriptPath, { KORCA_COPILOT_HOOK_EVENT: eventName })
          )
        ]
      }
      ...
      delete config.disableAllHooks
      config.hooks = nextHooks
      // Why: SSH remotes use POSIX scripts regardless of Korca's local OS. Write
      // the script before hooks/korca.json so a partial install cannot point
