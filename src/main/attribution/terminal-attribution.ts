...
import { KORCA_GIT_COMMIT_TRAILER } from '../../shared/korca-attribution'

const ATTRIBUTION_ROOT_DIR = 'korca-terminal-attribution'
const ATTRIBUTION_SHIM_VERSION = '6'
const KORCA_PRODUCT_URL = 'https://github.com/stablyai/korca'
const KORCA_GH_FOOTER = `Made with [Korca](${KORCA_PRODUCT_URL}) 🐋`
const SHELL_DOLLAR = '$'
const POWERSHELL_TICK = '`'
const ATTRIBUTION_ENV_KEYS = [
  'KORCA_ENABLE_GIT_ATTRIBUTION',
  'KORCA_GIT_COMMIT_TRAILER',
  'KORCA_GH_PR_FOOTER',
  'KORCA_GH_ISSUE_FOOTER',
...
export function applyTerminalAttributionEnv(
  ...
  // cannot recursively point KORCA_REAL_* at themselves.
  ...
  // Why: these wrappers should affect only Korca-managed PTYs. Prepending the
  // shim directory here keeps the attribution behavior scoped to Korca's live
  ...
  baseEnv.KORCA_ENABLE_GIT_ATTRIBUTION = '1'
  baseEnv.KORCA_GIT_COMMIT_TRAILER = KORCA_GIT_COMMIT_TRAILER
  baseEnv.KORCA_GH_PR_FOOTER = KORCA_GH_FOOTER
  baseEnv.KORCA_GH_ISSUE_FOOTER = KORCA_GH_FOOTER
  if (shellFamily === 'posix') {
    baseEnv.KORCA_ATTRIBUTION_SHIM_DIR = posixDir
  } else {
  ...
  if (platform === 'win32') {
    if (resolvedGit) {
      baseEnv.KORCA_REAL_GIT = resolvedGit
    }
    if (resolvedGh) {
      baseEnv.KORCA_REAL_GH = resolvedGh
    }
  }
}
...
function stripAttributionPathEntries(pathValue: string, pathDelimiter: string): string {
  return pathValue
    ...
    .filter((entry) => {
      ...
      return !normalized.includes('/korca-terminal-attribution/')
    })
    .join(pathDelimiter)
}
...
clean_path() {
  ...
  for entry in "${SHELL_DOLLAR}{entries[@]}"; do
    case "$entry" in
      "$script_dir"|*/korca-terminal-attribution/posix|*/korca-terminal-attribution/win32|*\\korca-terminal-attribution\\posix|*\\korca-terminal-attribution\\win32)
        ;;
      *)
        cleaned+=("$entry")
        ;;
...
if [[ -z "$real_git" ]]; then
  echo "Korca attribution wrapper could not locate git on PATH." >&2
  exit 127
fi
...
if [[ "\${KORCA_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${KORCA_ATTRIBUTION_BYPASS:-0}" == "1" ]] || ! is_commit_command "$@"; then
...
trailer="\${KORCA_GIT_COMMIT_TRAILER:-Co-authored-by: Korca <help@stably.ai>}"
...
KORCA_ATTRIBUTION_BYPASS=1 PATH="$real_path" exec "$real_git" "${SHELL_DOLLAR}{attributed_args[@]}"
`
...
if [[ -z "$real_gh" ]]; then
  echo "Korca attribution wrapper could not locate gh on PATH." >&2
  exit 127
fi
...
if [[ "\${KORCA_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${KORCA_ATTRIBUTION_BYPASS:-0}" == "1" ]]; then
  PATH="$real_path" exec "$real_gh" "$@"
fi

if [[ "\${1:-}" == "pr" && "\${2:-}" == "create" ]]; then
  footer="\${KORCA_GH_PR_FOOTER:-Made with [Korca](https://github.com/stablyai/korca) 🐋}"
...
if [[ "\${1:-}" == "issue" && "\${2:-}" == "create" ]]; then
  footer="\${KORCA_GH_ISSUE_FOOTER:-Made with [Korca](https://github.com/stablyai/korca) 🐋}"
  if has_passthrough_create_args "$@"; then
    PATH="$real_path" exec "$real_gh" "$@"
  fi
  if ! has_noninteractive_create_args "$@"; then
...
setlocal
if not "%KORCA_ENABLE_GIT_ATTRIBUTION%"=="1" goto run
if "%KORCA_ATTRIBUTION_BYPASS%"=="1" goto run
call :korca_is_git_commit %*
if errorlevel 1 goto run
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0git-wrapper.ps1" %*
exit /b %ERRORLEVEL%
:run
if defined KORCA_REAL_GIT (
  "%KORCA_REAL_GIT%" %*
) else (
  echo Korca attribution wrapper could not locate git on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
...
set "korca_git_arg=%~1"
if /I "%korca_git_arg%"=="-c" goto skip_two
if /I "%korca_git_arg%"=="--config" goto skip_two
if /I "%korca_git_arg%"=="-C" goto skip_two
if /I "%korca_git_arg%"=="--git-dir" goto skip_two
if /I "%korca_git_arg%"=="--work-tree" goto skip_two
if /I "%korca_git_arg%"=="--namespace" goto skip_two
if /I "%korca_git_arg:~0,9%"=="--config=" goto skip_one
if /I "%korca_git_arg:~0,10%"=="--git-dir=" goto skip_one
if /I "%korca_git_arg:~0,12%"=="--work-tree=" goto skip_one
if /I "%korca_git_arg:~0,12%"=="--namespace=" goto skip_one
if "%korca_git_arg:~0,1%"=="-" goto skip_one
exit /b 1
:skip_two
shift
shift
goto korca_is_git_commit
:skip_one
shift
goto korca_is_git_commit
`
...
exit /b %ERRORLEVEL%
:run
if defined KORCA_REAL_GH (
  "%KORCA_REAL_GH%" %*
) else (
  echo Korca attribution wrapper could not locate gh on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
`
...
$realGit = if ($env:KORCA_REAL_GIT) { $env:KORCA_REAL_GIT } else { 'git' }
$trailer = if ($env:KORCA_GIT_COMMIT_TRAILER) { $env:KORCA_GIT_COMMIT_TRAILER } else { 'Co-authored-by: Korca <help@stably.ai>' }
...
$env:KORCA_ATTRIBUTION_BYPASS = '1'
try {
...
$realGh = if ($env:KORCA_REAL_GH) { $env:KORCA_REAL_GH } else { 'gh' }
...
if ($isPrCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:KORCA_GH_PR_FOOTER) { $env:KORCA_GH_PR_FOOTER } else { 'Made with [Korca](https://github.com/stablyai/korca) 🐋' }
...
if ($isIssueCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:KORCA_GH_ISSUE_FOOTER) { $env:KORCA_GH_ISSUE_FOOTER } else { 'Made with [Korca](https://github.com/stablyai/korca) 🐋' }
    if ($null -ne $body -and $body -notmatch [Regex]::Escape($footer)) {
      $tmpFile = [System.IO.Path]::GetTempFileName()
      try {
        $trimmed = $body.TrimEnd("${POWERSHELL_TICK}r", "${POWERSHELL_TICK}n")
