...
describe('applyTerminalAttributionEnv', () => {
  ...
  function makeTmpRoot(): string {
    tmpRoot = mkdtempSync(join(tmpdir(), 'korca-attribution-'))
    return tmpRoot
  }
  ...
  function stripInheritedAttributionPath(pathValue: string): string {
    ...
    return pathValue
      .split(pathDelimiter)
      .filter((entry) => !entry.includes('korca-terminal-attribution'))
      .join(pathDelimiter)
  }

  function cleanAttributionEnv(env?: Record<string, string>): Record<string, string> {
    const base = { ...process.env }
    delete base.KORCA_ENABLE_GIT_ATTRIBUTION
    delete base.KORCA_GIT_COMMIT_TRAILER
    delete base.KORCA_GH_PR_FOOTER
    delete base.KORCA_GH_ISSUE_FOOTER
    delete base.KORCA_ATTRIBUTION_SHIM_DIR
    delete base.KORCA_REAL_GIT
    delete base.KORCA_REAL_GH
  ...
  it('does not amend HEAD when git commit --dry-run exits successfully', () => {
    ...
    mkdirSync(repo)
    runGit(repo, ['init'])
    runGit(repo, ['config', 'user.name', 'Korca Test'])
    runGit(repo, ['config', 'user.email', 'korca-test@example.com'])
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).not.toContain('Co-authored-by: Korca')
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).toContain(
      'Co-authored-by: Korca <help@stably.ai>'
    )
  })
...
`,
    ...
    expect(readFileSync(argsPath, 'utf8')).not.toContain('Co-authored-by: Korca')
  })
...
grep -Fq 'Co-authored-by: Korca <help@stably.ai>' "$1"
`,
      'utf8'
    )
...
`,
    ...
    expect(readFileSync(argsPath, 'utf8')).toContain('Co-authored-by: Korca <help@stably.ai>')
  })
...
if [[ "$1 $2 $3 $4" == "pr view --json url" ]]; then
  printf '%s\\n' 'https://github.com/stablyai/korca/pull/123'
  exit 0
fi
if [[ "$1 $2" == "api repos/stablyai/korca/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Existing body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/stablyai/korca/pulls/123" ]]; then
  touch "${markerPath}"
  exit 0
fi
exit 1
...
if [[ "$1 $2" == "issue create" ]]; then
  printf '%s\\n' 'https://github.com/stablyai/korca/issues/456'
  exit 0
fi
if [[ "$1 $2" == "api repos/stablyai/korca/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'PR body'
...
if [[ "$1 $2" == "api repos/stablyai/korca/issues/456" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Issue body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/stablyai/korca/pulls/123" ]]; then
...
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/stablyai/korca/issues/456" ]]; then
...
`,
    ...
    ).toBe('https://github.com/stablyai/korca/pull/123\n')
    expect(
      execFileSync('gh', ['issue', 'create', '--title', 'Issue', '--body', 'Body'], {
        encoding: 'utf8',
        env: cleanAttributionEnv(attributionEnv)
      })
    ).toBe('https://github.com/stablyai/korca/issues/456\n')
...
`,
    ...
    expect(output).toBe('https://github.com/stablyai/korca/pull/123\n')
    expect(existsSync(markerPath)).toBe(false)
  })
...
`,
  ...
  it('fails open when shim files cannot be written', () => {
    ...
    expect(baseEnv.KORCA_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
  ...
  it('does not duplicate shim directories when applied to an already-injected env', () => {
    ...
    const shimEntries = baseEnv.PATH.split(pathDelimiter).filter((entry) =>
      entry.includes('korca-terminal-attribution')
    )
  ...
  it('puts only Windows shims on PATH for native Windows shells', () => {
    ...
    const posixDir = join(userDataPath, 'korca-terminal-attribution', 'posix')
    const win32Dir = join(userDataPath, 'korca-terminal-attribution', 'win32')
    const pathEntries = baseEnv.PATH.split(';')

    expect(pathEntries[0]).toBe(win32Dir)
    expect(pathEntries).not.toContain(posixDir)
    expect(baseEnv.KORCA_ATTRIBUTION_SHIM_DIR).toBeUndefined()
  ...
  it('keeps POSIX shims first for Windows Git Bash and WSL shells', () => {
    ...
    expect(baseEnv.KORCA_ATTRIBUTION_SHIM_DIR).toBe(posixDir)
  ...
  it('writes PowerShell wrappers without raw-template backslash escapes', () => {
    ...
    const shimDir = join(root, 'user-data', 'korca-terminal-attribution', 'win32')
