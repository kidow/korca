...
const KORCA_CLAUDE_SERVICE = 'Korca Claude Code Managed Credentials'
...
export async function readManagedClaudeKeychainCredentials(
  ...
  return readKeychainPassword(KORCA_CLAUDE_SERVICE, accountId)
}
...
export async function writeManagedClaudeKeychainCredentials(
  ...
  await writeKeychainPassword(KORCA_CLAUDE_SERVICE, accountId, contents)
}

export async function deleteManagedClaudeKeychainCredentials(accountId: string): Promise<void> {
  await deleteKeychainPassword(KORCA_CLAUDE_SERVICE, accountId)
}
