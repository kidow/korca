...
describe('ClaudeRuntimeAuthService', () => {
  beforeEach(() => {
    ...
    testState.userDataDir = mkdtempSync(join(tmpdir(), 'korca-claude-runtime-'))
    testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'korca-claude-home-'))
  ...
  it('adopts canonical legacy managed auth paths without existing markers', async () => {
    ...
    const markerPath = join(managedAuthPath, '.korca-managed-claude-auth')
    expect(readFileSync(runtimeCredentialsPath, 'utf-8')).toBe(managedCredentials)
  ...
  it('restores system auth when switching from an owned account to missing credentials', async () => {
    ...
    writeFileSync(join(managedAuthPath2, '.korca-managed-claude-auth'), 'account-2\n', 'utf-8')
    writeFileSync(
  ...
  it('routes refreshed Claude credentials to the matching managed account', async () => {
    ...
    // Korca selected account-2. Persist that refresh to account-1, then restore
    ...
    const settings = createSettings({
      claudeManagedAccounts: [
        createClaudeAccount('account-1', managedAuthPath, {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/alice/.local/share/korca/claude-accounts/account-1/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
  ...
  it('uses the default distro selection for WSL-default Claude preparation', async () => {
    ...
    const settings = createSettings({
      claudeManagedAccounts: [
        createClaudeAccount('ubuntu-account', ubuntuAuthPath, {
          managedAuthRuntime: 'wsl',
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/alice/.local/share/korca/claude-accounts/ubuntu/auth'
        }),
        createClaudeAccount('debian-account', debianAuthPath, {
          managedAuthRuntime: 'wsl',
          wslDistro: 'Debian',
          wslLinuxAuthPath: '/home/alice/.local/share/korca/claude-accounts/debian/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
    ...
    try {
      ...
      expect(preparation).toMatchObject({
        runtime: 'wsl',
        wslDistro: 'Ubuntu',
        wslLinuxConfigDir: '/home/alice/.local/share/korca/claude-accounts/ubuntu/auth',
