...
import { getKorcaManagedCodexHomePath, getSystemCodexHomePath } from '../codex/codex-home-paths'
...
export function getCodexSessionsDirectory(): string {
  // Why: Korca-launched Codex processes receive an Korca-owned CODEX_HOME, so
  // callers that need the primary runtime path should not consult ambient
  // shell CODEX_HOME.
  return join(getKorcaManagedCodexHomePath(), 'sessions')
}

export function getCodexSessionDirectories(): string[] {
  // Why: upgraded users still have ordinary Codex history under ~/.codex, while
  // new Korca-launched sessions are written under Korca's managed runtime home.
...
export async function attributeCodexUsageEvent(
  ...
  if (event.cwd) {
    ...
    if (worktree) {
      ...
      // Why: all-local mode should still collapse repeated off-Korca sessions by
