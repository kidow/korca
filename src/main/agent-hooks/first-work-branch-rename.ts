...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Korca-created worktrees whose branch Korca is allowed to rename. */
  canRenameKorcaCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameKorcaCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`)
  }
  // Only rename a branch Korca auto-named — never one the user chose.
