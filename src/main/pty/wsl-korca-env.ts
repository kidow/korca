...
export function addKorcaWslInteropEnv(env: Record<string, string>): void {
  const entries = parseWslenvEntries(env.WSLENV)
  if (!hasWslenvVariable(entries, 'KORCA_TERMINAL_HANDLE')) {
    ...
    entries.push('KORCA_TERMINAL_HANDLE/u')
  }
