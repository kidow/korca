...
vi.mock('../hooks', () => ({
  ...
  parseKorcaYaml: vi.fn().mockReturnValue(null)
}))
...
afterEach(() => {
  ...
  vi.mocked(parseKorcaYaml).mockReset()
  ...
  vi.mocked(parseKorcaYaml).mockReturnValue(null)
...
function antigravityReadyScreen(model = 'Gemini 3.5 Flash (High)'): string {
  return [
    ...
    model,
    '~/korca/workspaces/korca/agy-dispatch-issue',
    '>'
  ].join('\n')
}
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.KORCA_TAB_ID).toMatch(UUID_RE)
  const leafId = env.KORCA_PANE_KEY?.slice(`${env.KORCA_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.KORCA_PANE_KEY).toBe(`${env.KORCA_TAB_ID}:${leafId}`)
  return env.KORCA_PANE_KEY
}

function createRuntime(): KorcaRuntimeService {
  return new KorcaRuntimeService(store)
}
...
describe('KorcaRuntimeService', () => {
  ...
  it('routes SSH-backed forward-slash UNC file and git paths without collapsing the root', async () => {
    ...
    const runtime = new KorcaRuntimeService(remoteStore as never)
  ...
  it('lists persisted SSH worktrees while the git provider is unavailable', async () => {
    ...
    const runtime = new KorcaRuntimeService(runtimeStore as never)
  ...
  it('creates additional workspace metadata for folder-mode repos through runtime create', async () => {
    ...
    const runtime = new KorcaRuntimeService(runtimeStore as never, undefined, {
  ...
  it('launches SSH setup terminals for runtime task-created worktrees', async () => {
    ...
    const provider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse') {
          return {
            stdout: '/remote/repo/.git/worktrees/mobile-setup/korca/setup-runner.sh\n',
            stderr: ''
          }
        }
        throw new Error(`unexpected git call: ${args.join(' ')}`)
    ...
    try {
      ...
      expect(result.setup).toMatchObject({
        runnerScriptPath: '/remote/repo/.git/worktrees/mobile-setup/korca/setup-runner.sh'
      })
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          cwd: '/remote/mobile-setup',
          command: 'bash /remote/repo/.git/worktrees/mobile-setup/korca/setup-runner.sh',
          worktreeId: result.worktree.id
        })
      )
  ...
  it('reads SSH repo hooks through the SSH filesystem provider', async () => {
    ...
    vi.mocked(parseKorcaYaml).mockReturnValue({ scripts: { setup: 'pnpm install' } })
    registerSshFilesystemProvider('ssh-1', fsProvider as never)
    const runtime = new KorcaRuntimeService(remoteStore as never)
  ...
  it('hashes only the shared korca.yaml setup script for local run-both hooks', async () => {
  ...
  it('uses remote path joins for SSH hook checks and issue-command files', async () => {
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => ({
        content: filePath.endsWith('korca.yaml')
          ? 'scripts:\n  setup: pnpm install\n'
    ...
    try {
      ...
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: 'C:\\remote\\repo\\.korca\\issue-command'
      })
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\korca.yaml')
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\.korca\\issue-command')
    expect(fsProvider.createDir).toHaveBeenCalledWith('C:\\remote\\repo\\.korca')
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      'C:\\remote\\repo\\.korca\\issue-command',
      'Ship it\n'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
  ...
  it('resolves SSH issue commands from shared korca.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseKorcaYaml).mockReturnValue({
      scripts: {},
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        ...
        if (filePath.endsWith('korca.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.korca/issue-command',
        source: 'shared'
      })
    ...
    expect(fsProvider.deletePath).toHaveBeenCalledWith('/remote/repo/.korca/issue-command', false)
  ...
  it('rejects hosted review worktree selectors outside the selected repo', async () => {
    ...
    const runtime = new KorcaRuntimeService(multiRepoStore as never)
  ...
  it('deduplicates runtime repo paths with Windows/UNC comparison semantics', async () => {
    ...
    const runtime = new KorcaRuntimeService(uncStore as never)
  ...
  it('defaults runtime addRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new KorcaRuntimeService(colorStore as never)
  ...
  it('defaults runtime createRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new KorcaRuntimeService(colorStore as never)
    const parentDir = await mkdtemp('/tmp/korca-runtime-create-')
    try {
  ...
  it('rejects runtime cloneRepo dot-segment URLs before spawning git', async () => {
    const spawnSpy = vi.spyOn(gitRunner, 'wslAwareSpawn')
    const runtime = createRuntime()
    const tempRoot = await mkdtemp(join(tmpdir(), 'korca-runtime-clone-'))
  ...
  it('removes an owned runtime clone target when git exits unsuccessfully', async () => {
    ...
    const runtime = createRuntime()
    const destination = await mkdtemp(join(tmpdir(), 'korca-runtime-clone-'))
  ...
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    // pre-allocate the tabId and stamp KORCA_PANE_KEY/TAB_ID/WORKTREE_ID into
    ...
    const spawnedLeafId = spawnedEnv.KORCA_PANE_KEY.slice(`${spawnedEnv.KORCA_TAB_ID}:`.length)
    expect(spawnedEnv.KORCA_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ptyId: 'pty-bg',
      title: 'worker',
      activate: false,
      tabId: spawnedEnv.KORCA_TAB_ID,
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    expect(spawnedEnv.KORCA_TAB_ID).toBe(tabId)
    expect(spawnedEnv.KORCA_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.KORCA_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.KORCA_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.KORCA_PANE_KEY).toMatch(`${spawnedEnv.KORCA_TAB_ID}:`)
  })
  ...
  it('splits visible pty-backed terminal sessions through the parent renderer tab', async () => {
    ...
    const sourceEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const sourceLeafId = sourceEnv.KORCA_PANE_KEY.slice(`${sourceEnv.KORCA_TAB_ID}:`.length)

    await expect(runtime.splitTerminal(handle, { direction: 'vertical' })).resolves.toMatchObject({
      handle: expect.stringMatching(/^term_/),
      tabId: sourceEnv.KORCA_TAB_ID,
      paneRuntimeId: -1
    })

    const splitEnv =
      (spawn.mock.calls[1]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    const splitLeafId = splitEnv.KORCA_PANE_KEY.slice(`${sourceEnv.KORCA_TAB_ID}:`.length)
    expect(splitTerminal).not.toHaveBeenCalled()
    expect(splitEnv.KORCA_TAB_ID).toBe(sourceEnv.KORCA_TAB_ID)
    expect(splitEnv.KORCA_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
  ...
  it('resolves tui-idle from a Codex ready prompt preview', async () => {
    ...
    runtime.onPtyData(
      ...
      [
        ...
        ' directory:   ~/korca/workspaces/korca/cli-debug\n'
      ].join(''),
      Date.now()
    )
    ...
    runtime.onPtyData(
      ...
      [
        ...
        [
          ...
          'Run /review on my current changes gpt-5.5 high ~/korca/workspaces/korca/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/korca/workspaces/korca/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/korca/workspaces/korca/cli-debug',
          'Run /review on my current changes gpt-5.5 high ~/korca/workspaces/korca/cli-debug\n'
        ].join('')
  ...
  // Why: PR #2553 fixed Korca CLI terminal reads so older retained output stays
  ...
  it('adopts preallocated KORCA_TERMINAL_HANDLE as a valid runtime handle', async () => {
    const runtime = new KorcaRuntimeService(store)
  ...
  it('reports browser tab creation as unsupported for headless runtime servers', async () => {
    ...
    await expect(
      ...
      code: 'browser_error',
      message: expect.stringContaining('headless korca serve')
    })
  })
  ...
  it('strips Korca provenance fields from runtime metadata updates', async () => {
    const metaById: Record<string, WorktreeMeta> = {
    ...
    await runtime.updateManagedWorktreeMeta(`id:${TEST_WORKTREE_ID}`, {
      ...
      korcaCreatedAt: 123,
      korcaCreationSource: 'runtime',
      korcaCreationWorkspaceLayout: { path: '/tmp', nestWorkspaces: false }
    })
  ...
  it('keeps cwd-inferred lineage best-effort when the cwd parent cannot be resolved', async () => {
    ...
    expect(result.warnings).toEqual([
      expect.objectContaining({
        ...
        message:
          'Worktree created, but Korca could not validate the current directory as a parent workspace.'
      })
    ])
  })
  ...
  it('returns a setup launch payload for CLI-created worktrees when hooks are explicitly enabled', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        ...
        KORCA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-test'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('passes setup payloads through when explicitly activating CLI-created worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KORCA_ROOT_PATH: '/tmp/repo',
        KORCA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-activate'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
  ...
  it('follows normal setup policy for CLI-created worktrees without activating them', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KORCA_ROOT_PATH: '/tmp/repo',
        KORCA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        ...
        // Why: createTerminal stamps KORCA_PANE_KEY/TAB_ID/WORKTREE_ID into the
        // PTY env on top of the caller-supplied env so hook-based agent status
        // can attribute hook events to a stable pane.
        env: expect.objectContaining({
          KORCA_ROOT_PATH: '/tmp/repo',
          KORCA_WORKTREE_PATH: '/tmp/workspaces/runtime-hook-skip',
          KORCA_TAB_ID: expect.stringMatching(UUID_RE),
          KORCA_PANE_KEY: expect.any(String),
          KORCA_WORKTREE_ID: result.worktree.id
        }),
        worktreeId: result.worktree.id
      })
    )
    ...
    const setupLeafId = setupSpawnEnv.KORCA_PANE_KEY.slice(`${setupSpawnEnv.KORCA_TAB_ID}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(result.worktree.id, {
      ptyId: 'pty-setup',
      title: 'Setup',
      activate: false,
      tabId: setupSpawnEnv.KORCA_TAB_ID,
      leafId: setupLeafId
    })
  })
  ...
  it('creates the first terminal for CLI-created worktrees without activating them', async () => {
    ...
    const initialLeafId = initialSpawnEnv.KORCA_PANE_KEY.slice(
      `${initialSpawnEnv.KORCA_TAB_ID}:`.length
    )
    expect(revealTerminalSession).toHaveBeenCalledWith(result.worktree.id, {
      ptyId: 'pty-created-worktree',
      title: null,
      activate: false,
      tabId: initialSpawnEnv.KORCA_TAB_ID,
  ...
  it('uses desktop task agent selection and bracketed-pastes startup drafts for local worktrees', async () => {
    ...
    const draftUrl = 'https://github.com/stablyai/korca/issues/123'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  it('records the resolved fallback agent when the requested startup draft agent is disabled', async () => {
    ...
    const result = await runtime.createManagedWorktree({
      ...
      startupDraft: 'https://github.com/stablyai/korca/issues/456',
      createdWithAgent: 'codex',
      activate: true
    })
  ...
  it('honors split setup placement for local startup-draft worktrees', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        KORCA_ROOT_PATH: '/tmp/repo',
        KORCA_WORKTREE_PATH: '/tmp/workspaces/runtime-startup-setup-split'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(mainEnv.KORCA_TAB_ID).toBeDefined()
    expect(mainEnv.KORCA_PANE_KEY).toBeDefined()
    expect(setupEnv.KORCA_TAB_ID).toBe(mainEnv.KORCA_TAB_ID)
    const mainLeafId = mainEnv.KORCA_PANE_KEY!.slice(`${mainEnv.KORCA_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-startup-split-setup',
        tabId: mainEnv.KORCA_TAB_ID,
        activate: false,
  ...
  it('lets explicit startup draft agents override the desktop default', async () => {
    ...
    const draftUrl = 'https://github.com/stablyai/korca/issues/789'
    const result = await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  it('detects agents on the SSH host before launching remote startup drafts', async () => {
    ...
    const draftUrl = 'https://github.com/stablyai/korca/pull/456'
    const result = await runtime.createManagedWorktree({
      repoSelector: TEST_REPO_ID,
  ...
  it('pre-marks remote Codex workspaces trusted before pasting startup drafts', async () => {
    ...
    try {
      const result = await runtime.createManagedWorktree({
        ...
        startupDraft: 'https://github.com/stablyai/korca/issues/789'
      })
  ...
  it('normalizes runtime GitLab issue list arguments like the desktop IPC path', async () => {
    const runtime = new KorcaRuntimeService(store as never)
  ...
  it('skips archive hooks for CLI worktree removal by default', async () => {
    ...
    expect(result.warning).toBe(
      `korca.yaml archive hook skipped for ${TEST_WORKTREE_PATH}; pass --run-hooks to run it.`
    )
  })
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'korca-runtime-remove-'))
  ...
  it('force-removes a legacy Korca-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'korca-runtime-orphan-'))
    ...
    const runtime = new KorcaRuntimeService(runtimeStoreWithRepoPath as never)
    const notifier = { worktreesChanged: vi.fn() }
    runtime.setNotifier(notifier as never)
  ...
  it('does not inspect or delete a local path when SSH runtime orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'korca-runtime-ssh-missing-fs-'))
    const repo = {
  ...
  it('still rejects forced runtime unregistered delete paths that exist on disk', async () => {
    const existingWorktreePath = await mkdtemp(join(tmpdir(), 'korca-runtime-remove-existing-'))
  ...
  async function reconcileWithToken(runtime: KorcaRuntimeService, token: string): Promise<void> {
    await runtime.reconcileWorktreeBaseStatus({
      repoId: TEST_REPO_ID,
      repoPath: TEST_REPO_PATH,
      worktreeId: TEST_WORKTREE_ID,
  ...
  it('preserves create-time metadata on later runtime listings when Windows path formatting differs', async () => {
    ...
    const runtime = new KorcaRuntimeService(runtimeStore)
    await runtime.createManagedWorktree({
      repoSelector: 'id:repo-1',
  ...
  describe('removeManagedWorktree PTY teardown (design §4.3)', () => {
    ...
    it('RPC-initiated delete kills matching PTYs before git', async () => {
      ...
      const runtime = new KorcaRuntimeService(store, undefined, {
        getLocalProvider: () => {
    ...
    it('thunk resolves the installed provider lazily, not at construction time', async () => {
      // Simulates the daemon adapter being installed AFTER KorcaRuntimeService
