...
import { RuntimeBrowserCommands } from './korca-runtime-browser'
import { RuntimeFileCommands } from './korca-runtime-files'
import { RuntimeGitCommands } from './korca-runtime-git'
...
export class KorcaRuntimeService {
  ...
  // RPC, but agents need their own handle at spawn time (via KORCA_TERMINAL_HANDLE
  ...
  // Why: daemon-backed PTYs that the runtime adopted after an Korca relaunch
  ...
  async addRepo(path: string, kind: 'git' | 'folder' = 'git'): Promise<Repo> {
    ...
    if (!isAbsolute(path)) {
      ...
      // server-side repo paths to be explicit so `korca serve` cwd is irrelevant.
      throw new Error('Project path must be an absolute path')
    }
  ...
  async getRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        const hooks = result.isBinary ? null : parseKorcaYaml(result.content)
        return {
          hasHooksFile: Boolean(hooks),
          hooks,
          setupRunPolicy: getEffectiveSetupRunPolicy(repo),
          source: hooks ? 'korca.yaml' : null,
    ...
    return {
      ...
      hooks,
      setupRunPolicy,
      source: hasFile ? 'korca.yaml' : hooks ? 'legacy' : null,
      setupTrust: this.getSharedSetupHookTrustPayload(
        repo,
  ...
  async checkRepoHooks(repoSelector: string) {
    ...
    if (repo.connectionId) {
      ...
      try {
        ...
        return { hasHooks: true, hooks: parseKorcaYaml(result.content), mayNeedUpdate: false }
      } catch {
        return { hasHooks: false, hooks: null, mayNeedUpdate: false }
      }
    }
    ...
    return {
      hasHooks: has,
      hooks,
      mayNeedUpdate: has && !hooks && hasUnrecognizedKorcaYamlKeys(repo.path)
    }
  }
  ...
  private async readRemoteSharedIssueCommand(
    ...
    try {
      const result = await fsProvider.readFile(joinWorktreeRelativePath(repoPath, 'korca.yaml'))
      if (result.isBinary) {
        return null
      }
      return parseKorcaYaml(result.content)?.issueCommand?.trim() || null
    } catch {
      return null
    }
  }
  ...
  async writeRepoIssueCommand(repoSelector: string, content: string): Promise<{ ok: true }> {
    ...
    if (repo.connectionId) {
      ...
      await this.ensureRemoteKorcaDirIgnored(fsProvider, repo.path)
  ...
  private async ensureRemoteKorcaDirIgnored(
    fsProvider: IFilesystemProvider,
    repoPath: string
  ): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.warn('[runtime] Could not update remote .gitignore to exclude .korca', error)
      }
    }
  }
  ...
  async listDetectedManagedWorktrees(repoSelector: string): Promise<DetectedWorktreeListResult> {
    ...
    const detected = scan.worktrees.map((gitWorktree) => {
      ...
      return {
        ...detectedWorktree,
        visible: true,
        ownership: detectedWorktree.ownership === 'korca-managed' ? 'korca-managed' : 'unknown-legacy'
  ...
  private toRuntimeDetectedWorktree(repo: Repo, worktree: Worktree): DetectedWorktree {
    ...
    return toDetectedWorktree({
      ...
      settings,
      knownKorcaLayouts: buildKnownKorcaWorkspaceLayouts(settings, repo),
  ...
  async createManagedWorktree(args: {
    ...
    // Why: CLI-managed WSL worktrees live under ~/korca/workspaces inside the
    ...
    const displayNameMeta = requestedDisplayName
      ...
      : shouldSetDisplayName(effectiveRequestedName, branchName, effectiveSanitizedName)
        ...
        : {}
    const meta = this.store.setWorktreeMeta(worktreeId, {
      // Why: worktree IDs are path-derived. If a path is deleted outside Korca
    ...
    if (lineageResolution.kind === 'lineage') {
      ...
      if (childInstanceId && parentInstanceId && this.store.setWorktreeLineage) {
        ...
        lineageWarnings.push({
          ...
          message:
            'Worktree created, but Korca could not record lineage because instance identity was unavailable.',
          details: {
    ...
    // defers to the repo's korca.yaml setupRunPolicy. runHooks === true maps
    ...
    if (shouldRunSetup && hooks?.scripts.setup) {
      if (this.authoritativeWindowId !== null) {
        try {
          ...
          // renderer create flow so repo-committed `korca.yaml` setup hooks run in
      ...
      warning = `korca.yaml setup hook skipped for ${worktreePath}; pass --run-hooks to run it.`
    ...
    if (effectiveStartup && this.ptyController?.spawn) {
      try {
        ...
        // session later, matching `korca terminal create` background semantics.
        const startupTrustAgent = effectiveDraftPaste?.agent ?? effectiveCreatedWithAgent
        if (startupTrustAgent) {
  ...
  async getOrStartRemoteFetch(repoPath: string, remote: string): Promise<RemoteFetchResult> {
    ...
    const promise = this.enqueueRemoteFetch(key, () =>
      ...
      // create on this repo until Korca restarted (the F2 bug §3.3 pins).
  ...
  async updateManagedWorktreeMeta(
    ...
    this.store.setWorktreeMeta(
      worktree.id,
      stripKorcaProvenanceMetaUpdates(omitUndefinedProperties(metaUpdates))
    )
  ...
  private async resolveWorktreeRemovalTarget(
    ...
    try {
      ...
      // Only exact IDs with persisted Korca metadata are accepted here so
  ...
  async removeManagedWorktree(
    ...
    const removal = (async (): Promise<RemoveWorktreeResult & { warning?: string }> => {
      ...
      if (hooks?.scripts.archive && runHooks) {
        ...
        // Runtime RPC calls have no renderer trust prompt, so hooks require explicit CLI opt-in.
        warning = `korca.yaml archive hook skipped for ${canonicalWorktreePath}; pass --run-hooks to run it.`
        console.warn(`[hooks] ${warning}`)
      }
  ...
  async createTerminal(
    ...
    const shouldCreateInBackground =
      ...
      ((opts.focus !== true && opts.rendererBacked !== true) ||
        // Why: `korca serve` exposes the local runtime without a renderer
  ...
  markRendererReloading(windowId: number): void {
    ...
    // Why: handleByPtyId maps ptyId → pre-allocated CLI handle (KORCA_TERMINAL_HANDLE).
  ...
  markGraphUnavailable(windowId: number): void {
    ...
    // Why: once the authoritative renderer graph disappears, Korca must fail
    ...
    if (input.callerTerminalHandle) {
      try {
        ...
        warnings.push({
          code: 'LINEAGE_PARENT_CONTEXT_MISSING',
          message:
            'Worktree created, but Korca could not validate the caller terminal as a parent workspace.',
        ...
        warnings: [
          {
            ...
            message: 'Worktree created, but Korca could not prove which parent workspace caused it.',
            details: {
  ...
  private async computeResolvedWorktrees(generation: number): Promise<ResolvedWorktree[]> {
    ...
    // allowed to change outside Korca. A short TTL avoids shelling out on every
    ...
    for (const tab of snapshot.tabs) {
      if (tab.type === 'browser') {
        ...
        tabs.push({
          ...
          // active Korca tab. Preserve the renderer's app-level session focus.
  ...
  private rememberDetachedPreAllocatedLeaves(): void {
    for (const leaf of this.leaves.values()) {
      if (leaf.ptyId && this.handleByPtyId.has(leaf.ptyId)) {
        // Why: KORCA_TERMINAL_HANDLE is an agent identity, so CLI control should
...
function branchSelectorMatches(branch: string, selector: string): boolean {
  ...
  // or `foo` depending on which plumbing path produced the record. Korca's
