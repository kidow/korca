/* eslint-disable max-lines -- Why: this file is a command adapter for one external surface, Agent Browser automation. It stays separate from KorcaRuntimeService so runtime state does not grow further while browser routing remains easy to scan in one place. */
...
export class RuntimeBrowserCommands {
  ...
  // listeners never fire, leaving the Zustand store (and thus the Korca UI's
  ...
  async browserTabCreate(params: {
    ...
    if (!this.host.getAvailableAuthoritativeWindow()) {
      throw new BrowserError(
        'browser_error',
        'Browser tab creation requires a desktop renderer; headless korca serve does not support browser panes yet.'
      )
    }
