...
type KorcaRuntimeRpcServerOptions = {
...
export class KorcaRuntimeRpcServer {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // the `korca` CLI. Close all transports immediately instead of leaving
  ...
  async stop(): Promise<void> {
    ...
    // Korca processes during restarts, updates, or development, and stale
...
export function createRuntimeTransportMetadata(
  ...
  if (platform === 'win32') {
    return {
      ...
      endpoint: `\\\\.\\pipe\\korca-${pid}-${endpointSuffix}`
    }
  }
  return {
    kind: 'unix',
