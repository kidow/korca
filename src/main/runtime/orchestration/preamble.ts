...
// Why: the dispatch preamble teaches agents about Korca's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use korca-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Korca instance (Section 6.4).
  const cli = params.devMode ? 'korca-dev' : 'korca'

  const header = `You are working inside Korca, a multi-agent IDE. You are a dispatched worker.
