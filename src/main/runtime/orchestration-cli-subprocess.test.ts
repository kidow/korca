...
// real in-process runtime via KORCA_USER_DATA_PATH, and asserts:
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...
      KORCA_TERMINAL_HANDLE: 'term_cli',
      ...extraEnv
    },
...
describeIfBuilt('korca orchestration check --wait subprocess (§3.4)', () => {
  it('emits newline-flushed JSON heartbeats to stderr while waiting', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'korca-cli-sub-'))
    ...
    try {
      // Why: use the KORCA_HEARTBEAT_INTERVAL_MS escape hatch to shrink the
      ...
      const child = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            KORCA_USER_DATA_PATH: userDataPath,
            KORCA_TERMINAL_HANDLE: 'term_nobody',
            KORCA_HEARTBEAT_INTERVAL_MS: String(heartbeatMs)
          },
...
describeIfBuilt('korca orchestration reset subprocess', () => {
  it('validates reset scopes against an isolated runtime through the built CLI', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'korca-cli-reset-'))
    const runtime = new KorcaRuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
    const server = new KorcaRuntimeRpcServer({ runtime, userDataPath })
