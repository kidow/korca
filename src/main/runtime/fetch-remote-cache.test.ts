...
// `KorcaRuntimeService.fetchRemoteWithCache` — in particular that a rejected
...
// without disturbing the large korca-runtime.test.ts mock surface.
...
// Why: korca-runtime.ts imports heavy modules (hooks, ipc/*, etc.) at top
...
describe('KorcaRuntimeService.fetchRemoteWithCache', () => {
  beforeEach(() => {
  ...
  it('evicts the in-flight Map entry on rejection so the next caller re-fetches', async () => {
    ...
    const runtime = new KorcaRuntimeService(null)
