...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__korcaGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-korca-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-korca-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-korca-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-korca-previous-display') || '';
    el.removeAttribute('data-korca-previous-display');
  });
})()`
