...
describe('gitlab project ref parsing', () => {
  it('parses HTTPS and SSH GitLab.com remotes', () => {
    ...
    expect(parseGitLabProjectRef('git@gitlab.com:stablyai/korca.git')).toEqual({
      host: 'gitlab.com',
      path: 'stablyai/korca'
    })
  })
  ...
  it('returns null for non-GitLab hosts when host not in knownHosts', () => {
    expect(parseGitLabProjectRef('git@github.com:stablyai/korca.git')).toBeNull()
...
describe('gitlab project ref resolution', () => {
  ...
  it('keeps getProjectRef origin-based', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@gitlab.com:fork/korca.git\n'
    })
  ...
  it('prefers upstream for issue project ref resolution', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@gitlab.com:stablyai/korca.git\n'
    })
  ...
  it('falls back to origin when upstream is missing or non-GitLab', async () => {
    gitExecFileAsyncMock
      ...
      .mockResolvedValueOnce({ stdout: 'git@gitlab.com:fork/korca.git\n' })
  ...
  it('does not mix origin and upstream cache entries for the same repo path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@gitlab.com:fork/korca.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@gitlab.com:stablyai/korca.git\n' })
  ...
  it('resolves project refs through the SSH git provider for connected repos', async () => {
    sshExecMock.mockResolvedValueOnce({ stdout: 'git@gitlab.com:remote/korca.git\n', stderr: '' })
  ...
  it('does not cache a missing SSH provider as a permanent null project ref', async () => {
    ...
    sshExecMock.mockResolvedValueOnce({
      stdout: 'git@gitlab.com:remote/korca.git\n',
      stderr: ''
    })
    registerSshGitProvider('conn-1', { exec: sshExecMock } as never)
  ...
  it('does not cache transient SSH exec failures as permanent null project refs', async () => {
    sshExecMock
      ...
      .mockResolvedValueOnce({ stdout: 'git@gitlab.com:remote/korca.git\n', stderr: '' })
    registerSshGitProvider('conn-1', { exec: sshExecMock } as never)
...
describe('resolveIssueSource', () => {
  ...
  it("'auto' + upstream exists → upstream, fellBack=false", async () => {
    ...
    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { host: 'gitlab.com', path: 'stablyai/korca' },
      fellBack: false
    })
  })
  ...
  it("'auto' + no upstream → origin, fellBack=false", async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:stablyai/korca.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@gitlab.com:solo/korca.git\n' })

    await expect(resolveIssueSource('/repo', 'auto')).resolves.toEqual({
      source: { host: 'gitlab.com', path: 'solo/korca' },
      fellBack: false
    })
  })
  ...
  it("'origin' + upstream exists → origin (ignores upstream), fellBack=false", async () => {
    ...
    await expect(resolveIssueSource('/repo', 'origin')).resolves.toEqual({
      source: { host: 'gitlab.com', path: 'fork/korca' },
      fellBack: false
    })
