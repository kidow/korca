...
describe('resolveConsent', () => {
  ...
  it('returns korca_disabled when KORCA_TELEMETRY_DISABLED=1', () => {
    process.env.KORCA_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
      ...
      effective: 'disabled',
      reason: 'korca_disabled'
    })
  })

  it('prefers do_not_track over korca_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.KORCA_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'korca_disabled' })
  })
})
