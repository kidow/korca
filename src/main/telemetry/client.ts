...
const BUILD_IDENTITY: 'stable' | 'rc' | null =
  typeof KORCA_BUILD_IDENTITY !== 'undefined'
    ...
    : ((globalThis as { KORCA_BUILD_IDENTITY?: 'stable' | 'rc' | null }).KORCA_BUILD_IDENTITY ?? null)
const WRITE_KEY: string | null =
  typeof KORCA_POSTHOG_WRITE_KEY !== 'undefined'
    ? KORCA_POSTHOG_WRITE_KEY
    : ((globalThis as { KORCA_POSTHOG_WRITE_KEY?: string | null }).KORCA_POSTHOG_WRITE_KEY ?? null)
 ...
 * KORCA_TELEMETRY_DISABLED). The SDK flag does not persist across process
...
// official stable/rc builds where CI injects `KORCA_BUILD_IDENTITY` and
// `KORCA_POSTHOG_WRITE_KEY`.
