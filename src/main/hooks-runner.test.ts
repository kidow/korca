...
describe('createSetupRunnerScript', () => {
  ...
  it('writes a fail-fast Windows runner that returns after batch commands', async () => {
    ...
    execFileSyncMock.mockReturnValue('C:\\repo\\.git\\worktrees\\feature\\korca\\setup-runner.cmd')
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath: 'C:\\repo\\.git\\worktrees\\feature\\korca\\setup-runner.cmd',
        envVars: expect.objectContaining({
          KORCA_ROOT_PATH: '/test/repo',
          KORCA_WORKTREE_PATH: 'C:\\repo\\feature\\',
          KORCA_WORKSPACE_NAME: 'feature'
        })
      })
      expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith(
        'C:\\repo\\.git\\worktrees\\feature\\korca\\setup-runner.cmd',
        [
          '@echo off',
  ...
  it('derives KORCA_WORKSPACE_NAME from a POSIX worktree path', async () => {
    const originalPlatform = process.platform

    execFileSyncMock.mockReturnValue('/test/repo/.git/worktrees/feature/korca/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'linux'
    })
    ...
    try {
      ...
      expect(result.envVars).toEqual(
        expect.objectContaining({
          KORCA_WORKTREE_PATH: '/test/repo-feature',
          KORCA_WORKSPACE_NAME: 'repo-feature'
        })
      )
    } finally {
      Object.defineProperty(process, 'platform', {
  ...
  it('translates WSL runner paths and env vars to Linux form on Windows', async () => {
    ...
    execFileSyncMock.mockReturnValue('/home/jin/.git/worktrees/feature/korca/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath:
          '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.git\\worktrees\\feature\\korca\\setup-runner.sh',
        envVars: expect.objectContaining({
  ...
  it('translates WSL env vars to Linux paths when the worktree lives on a WSL UNC path', async () => {
    ...
    execFileSyncMock.mockReturnValue('/home/jin/repo/.git/worktrees/feature/korca/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        envVars: expect.objectContaining({
          KORCA_ROOT_PATH: '/test/repo',
          KORCA_WORKTREE_PATH: '/home/jin/repo/feature',
          KORCA_WORKSPACE_NAME: 'feature',
...
describe('createIssueCommandRunnerScript', () => {
  ...
  it('writes a POSIX runner under the worktree git dir for long issue commands', async () => {
    ...
    execFileSyncMock.mockReturnValue(
      '/test/repo/.git/worktrees/feature/korca/issue-command-runner.sh'
    )
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'linux'
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath: '/test/repo/.git/worktrees/feature/korca/issue-command-runner.sh',
        envVars: expect.objectContaining({
          KORCA_ROOT_PATH: '/test/repo',
          KORCA_WORKTREE_PATH: '/test/repo-feature'
        })
