...
function requestDevParentShutdown(): void {
  ...
  const forceExitTimer = setTimeout(() => {
    ...
    // handlers or window-close guards stall and would otherwise leave Korca
  ...
  if (process.platform !== 'win32') {
    ...
    if (home) {
      extraPaths.push(
        ...
        // user can run them from Terminal. See stablyai/korca#829.
...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  const overrideUserDataPath = process.env.KORCA_DEV_USER_DATA_PATH
  if (overrideUserDataPath) {
    // Why: automated Electron repros need an isolated profile so persisted
    // tabs/worktrees from the developer's normal `korca-dev` session do not
    // change startup behavior and hide or create window-management bugs.
    app.setPath('userData', overrideUserDataPath)
    return
  }
  // Why: development runs share the same machine as packaged Korca, and both
  ...
  // public `korca` CLI look broken even though the packaged app is still open.
  app.setPath('userData', join(app.getPath('appData'), 'korca-dev'))
}

export function shouldInstallManagedHooks(isDev: boolean): boolean {
  void isDev
  // Why: managed hook installation now targets Korca-owned, environment-scoped
...
export function installDevParentWatchdog(isDev: boolean): void {
  ...
  const timer = setInterval(() => {
    ...
    if (parentPidChanged || parentMissing) {
      ...
      // the dev runner while leaving Korca open. Watching the original parent PID
