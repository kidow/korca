...
describe('dev-instance-identity', () => {
  it('keeps packaged identity stable', () => {
    expect(getDevInstanceIdentity(false, {})).toMatchObject({
      ...
      devLabel: null,
      dockBadgeLabel: null,
      appUserModelId: 'com.stablyai.korca'
    })
  })

  it('derives a readable dev label from worktree and branch env', () => {
    const identity = getDevInstanceIdentity(true, {
      KORCA_DEV_REPO_ROOT: '/repo/worktrees/dev-indicator',
      KORCA_DEV_WORKTREE_NAME: 'dev-indicator',
      KORCA_DEV_BRANCH: 'nwparker/dev-indicator'
    })
    ...
    expect(identity.name).toBe('Korca: nwparker/dev-indicator')
    expect(identity.dockBadgeLabel).toBeNull()
    expect(identity.appUserModelId).toMatch(/^com\.stablyai\.korca\.dev\.[a-f0-9]{10}$/)
  })

  it('includes the branch when it differs from the worktree basename', () => {
    const identity = getDevInstanceIdentity(true, {
      KORCA_DEV_REPO_ROOT: '/repo/worktrees/payment-ui',
    ...
    expect(identity.name).toBe('Korca: feature/billing-shell')
    expect(identity.dockBadgeLabel).toBeNull()
  })

  it('allows an explicit label override', () => {
    const identity = getDevInstanceIdentity(true, {
      KORCA_DEV_INSTANCE_LABEL: 'manual label',
      KORCA_DEV_WORKTREE_NAME: 'dev-indicator',
      KORCA_DEV_BRANCH: 'feature/other'
    })

    expect(identity.devLabel).toBe('manual label')
    expect(identity.name).toBe('Korca: feature/other')
    expect(identity.dockBadgeLabel).toBeNull()
  })
})
