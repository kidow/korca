...
// `which` probe even though they work fine from Terminal (see stablyai/korca#829).
//
...
const DELIMITER = '__KORCA_SHELL_PATH__'
...
function spawnShellAndReadPath(shell: string): Promise<HydrationResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(shell, ['-ilc', command], {
      ...
      // and we don't want that in Korca's console.
      env: process.env,
