...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Korca instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Korca process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'KORCA_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] KORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Korca instance running for the same profile.'

/**
 * Why: Korca writes two canonical discovery files into `<userData>/`:
 * `korca-runtime.json` (RPC endpoint + authToken for the bundled CLI) and
 ...
 * instance quits, metadata points at a dead pid and `korca status` reports
 ...
 * way dev (`korca-dev` userData) and packaged (`korca` userData) runs lock in
