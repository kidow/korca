...
export function maybeRedirectAppImageCliLaunch(options: RedirectOptions = {}): RedirectResult {
  ...
  if (!existsSync(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Korca CLI entrypoint at ${cliEntryPath}\n`)
...
function buildElectronRunAsNodeEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const childEnv = { ...env }
  childEnv.KORCA_NODE_OPTIONS = env.NODE_OPTIONS ?? ''
  childEnv.KORCA_NODE_REPL_EXTERNAL_MODULE = env.NODE_REPL_EXTERNAL_MODULE ?? ''
