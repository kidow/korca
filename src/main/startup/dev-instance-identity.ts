...
const BASE_APP_NAME = 'Korca'
const BASE_APP_USER_MODEL_ID = 'com.stablyai.korca'
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.KORCA_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.KORCA_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.KORCA_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.KORCA_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.KORCA_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`
