...
import { getKorcaManagedCodexHomePath } from './codex/codex-home-paths'
 ...
 * first launch.
 *
 * Why: Korca's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: Korca-launched Codex runs with an Korca-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  upsertProjectTrustLevel(join(getKorcaManagedCodexHomePath(), 'config.toml'), absPath, 'trusted')
}
...
function canonicalize(p: string): string {
  ...
  // (korca caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
