...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('KORCA_OPENCODE_CONFIG_DIR')
    expect(script).toContain('KORCA_PI_CODING_AGENT_DIR')
    expect(script).toContain('KORCA_OMP_CODING_AGENT_DIR')
    expect(script).toContain('KORCA_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:KORCA_OMP_STATUS_EXTENSION')
    expect(script).toContain('KORCA_CODEX_HOME')
