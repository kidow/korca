...
function getKorcaDir(): string {
  return join(homedir(), '.korca')
}

function getLegacyTokenPath(): string {
  return join(getKorcaDir(), 'linear-token.enc')
}

function getLegacyViewerPath(): string {
  return join(getKorcaDir(), 'linear-viewer.json')
}

function getWorkspaceFilePath(): string {
  return join(getKorcaDir(), 'linear-workspaces.json')
}

function getWorkspaceTokenDir(): string {
  return join(getKorcaDir(), 'linear-tokens')
}
...
function ensureKorcaDir(): void {
  const dir = getKorcaDir()
  if (!existsSync(dir)) {
