...
describe('GitHub issue source split', () => {
  ...
  it('uses upstream for issues and origin for PRs in mixed recent results', async () => {
    ...
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'korca' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            title: 'Fork PR',
            state: 'open',
            html_url: 'https://github.com/fork/korca/pull/42',
            labels: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        'repos/stablyai/korca/issues?per_page=10&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        'repos/fork/korca/pulls?per_page=10&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('omits gh api cache args for no-cache recent work-item requests', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/stablyai/korca/issues?per_page=10&state=open&sort=updated&direction=desc'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/fork/korca/pulls?per_page=10&state=open&sort=updated&direction=desc'],
      { cwd: '/repo-root' }
    )
  })
  ...
  it('lists SSH repo work items with explicit owner/repo and no local cwd', async () => {
    ...
    await listWorkItems('/home/jinwoo/korca', 10, undefined, undefined, 'auto', 'openclaw-2')

    expect(resolveIssueSourceMock).toHaveBeenCalledWith('/home/jinwoo/korca', 'auto', 'openclaw-2')
    expect(getOwnerRepoMock).toHaveBeenCalledWith('/home/jinwoo/korca', 'openclaw-2')
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      [
        'api',
  ...
  it('uses upstream for issue-only queries and origin for PR-only queries', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'stablyai/korca']),
      { cwd: '/repo-root' }
    )
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'fork/korca']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it('counts default work items across upstream issues and origin PRs', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:stablyai/korca is:issue is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:fork/korca is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it('raw number lookup tries upstream issue before origin PR', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/stablyai/korca/issues/42'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
  ...
  it('surfaces a 403 from upstream issues through the listWorkItems envelope', async () => {
    ...
    expect(result.sources).toMatchObject({
      issues: { owner: 'stablyai', repo: 'korca' },
      prs: { owner: 'fork', repo: 'korca' }
    })
  ...
  describe('per-repo issue-source preference', () => {
    ...
    it("preference='auto' + no upstream → queries origin", async () => {
      ...
      getOwnerRepoMock.mockResolvedValueOnce({ owner: 'solo', repo: 'korca' })
      ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
        stdout: '[]'
      })
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
        ...
        [
          ...
          '--cache',
          '120s',
          'repos/solo/korca/issues?per_page=10&state=open&sort=updated&direction=desc'
        ],
        { cwd: '/repo-root' }
      )
    })
    ...
    it("preference='origin' + upstream exists → queries origin (not upstream)", async () => {
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
        1,
        expect.arrayContaining([
          'repos/fork/korca/issues?per_page=10&state=open&sort=updated&direction=desc'
        ]),
        { cwd: '/repo-root' }
      )
    })
    ...
    it('surfaces upstreamCandidate in sources regardless of effective preference', async () => {
      ...
      expect(result.sources).toEqual({
        issues: { owner: 'fork', repo: 'korca' },
        prs: { owner: 'fork', repo: 'korca' },
        upstreamCandidate: { owner: 'stablyai', repo: 'korca' }
      })
    })
  })
})
