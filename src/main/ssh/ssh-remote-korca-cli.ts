...
export type RemoteKorcaCliRequest = {
  argv: string[]
  cwd: string
  env: Record<string, string>
}

export type RemoteKorcaCliResult = {
  stdout: string
  stderr: string
  exitCode: number
}
...
export async function runRemoteKorcaCli(
  runtime: KorcaRuntimeService,
  request: RemoteKorcaCliRequest
): Promise<RemoteKorcaCliResult> {
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      throw new Error(`Unsupported SSH Korca CLI command: ${command}`)
  }
}
...
function resolveHandle(
  ...
  return optionalString(flags, flagName) ?? env.KORCA_TERMINAL_HANDLE ?? 'unknown'
}
