...
describe('SshRelaySession', () => {
  ...
  it('installs remote managed hooks and relay-owned plugin assets before registering the SSH PTY provider', async () => {
    process.env.KORCA_FEATURE_REMOTE_AGENT_HOOKS = '1'
    muxRequestMock.mockImplementation(async (method: string) => {
      if (method === 'session.resolveHome') {
        return { resolvedPath: '/home/korca' }
      }
      return { ok: true }
    })
    ...
    expect(installRemoteManagedAgentHooksMock).toHaveBeenCalledWith(sftp, '/home/korca')
