...
function makeHome(): string {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'korca-commit-env-'))
  tempDirs.push(dir)
...
describe('prepareLocalCommitMessageAgentEnv', () => {
  ...
  it('prefers the original OpenCode config root over inherited PTY overlays', async () => {
    process.env.OPENCODE_CONFIG_DIR = '/tmp/korca-opencode-overlay'
    process.env.KORCA_OPENCODE_SOURCE_CONFIG_DIR = '/Users/tester/company/opencode'
  ...
  it('prefers the original Pi agent root over inherited PTY overlays', async () => {
    process.env.PI_CODING_AGENT_DIR = '/tmp/korca-pi-overlay'
    process.env.KORCA_PI_SOURCE_AGENT_DIR = '/Users/tester/.pi/agent'
  ...
  it('sets CODEX_HOME for host managed Codex accounts', async () => {
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        'C:\\Users\\tester\\AppData\\Roaming\\Korca\\codex-accounts\\a\\home'
    })

    expect(result).toEqual({
      ok: true,
      env: expect.objectContaining({
        CODEX_HOME: 'C:\\Users\\tester\\AppData\\Roaming\\Korca\\codex-accounts\\a\\home'
      })
    })
  })
  ...
  it('does not pass WSL managed Codex homes to host-local commit generation', async () => {
    ...
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        '\\\\wsl.localhost\\Ubuntu\\home\\tester\\.local\\share\\korca\\codex-accounts\\a\\home'
    })
