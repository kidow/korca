...
function prepareShellConfigDirEnv(agentId: string): { ok: true; env?: NodeJS.ProcessEnv } | null {
  ...
  // Why: each kind owns a distinct KORCA_*_SOURCE_* shadow so a headless commit
  ...
  const sourceVar =
    agentId === 'opencode'
      ? 'KORCA_OPENCODE_SOURCE_CONFIG_DIR'
      : agentId === 'pi'
        ? 'KORCA_PI_SOURCE_AGENT_DIR'
        : agentId === 'omp'
          ? 'KORCA_OMP_SOURCE_AGENT_DIR'
          : undefined
  ...
  // Why: GUI-launched Korca may not inherit shell startup exports, but these
  // vars point the headless CLI at the user's auth/config root. Nested Korca
  // launches inherit PTY overlays, so prefer KORCA_*_SOURCE_* when present.
