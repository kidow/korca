...
const HERMES_PLUGIN_NAME = 'korca-status'
const HERMES_PLUGIN_MARKER = 'Managed by Korca. Do not edit; changes may be overwritten.'
...
function enablePlugin(config: HermesConfig): HermesConfig {
  ...
  if (disabled === null) {
    ...
    // so Korca's install status matches what the real Hermes loader will do.
    plugins.disabled = []
...
function getPluginFilesState(pluginDir = getPluginDir()): {
  ...
  try {
    ...
    return {
      present: true,
      managed,
      detail: managed ? null : 'Hermes korca-status plugin exists but is not Korca-managed'
    }
  } catch (error) {
    return {
      present: true,
...
function buildStatus(configPath: string, config: HermesConfig): AgentHookInstallStatus {
  ...
  const details = [
    pluginFiles.detail,
    enablement.detail,
    !enablement.enabled ? 'korca-status is not enabled in Hermes config.yaml' : null,
    enablement.disabled ? 'korca-status is disabled in Hermes config.yaml' : null
...
function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Korca."',
    'author: "Korca"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("KORCA_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_korca(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("KORCA_AGENT_HOOK_PORT", "")
    token = env.get("KORCA_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("KORCA_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "tabId": env.get("KORCA_TAB_ID", ""),
        "worktreeId": env.get("KORCA_WORKTREE_ID", ""),
        "env": env.get("KORCA_AGENT_HOOK_ENV", ""),
        "version": env.get("KORCA_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "X-Korca-Agent-Hook-Token": token,
        },
    )
    try:
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_korca(_payload_for_event(event_name, kwargs))
