...
describe('HermesHookService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = mkdtempSync(join(tmpdir(), 'korca-hermes-hooks-'))
  ...
  it('preserves other enabled plugins and removes Korca from disabled list', () => {
    writeFileSync(
  ...
  it('registered plugin hooks post normalized JSON to Korca', async () => {
    ...
    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            expect(req.headers['x-korca-agent-hook-token']).toBe('token-1')
            res.writeHead(204)
            res.end()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const script = [
          ...
          `spec = importlib.util.spec_from_file_location("korca_status", ${JSON.stringify(initPath)})`,
        ...
        execFile(
          ...
          ['-c', script],
          {
            env: {
              ...
              KORCA_AGENT_HOOK_TOKEN: 'token-1',
              KORCA_AGENT_HOOK_ENDPOINT: '',
              KORCA_PANE_KEY: PANE_KEY,
              KORCA_TAB_ID: 'tab-1',
