...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\resources\\bin\\korca.cmd'
) {
  return {
    platform: 'win32',
    commandName: 'korca',
    commandPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\bin\\korca.cmd',
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\bin',
    pathConfigured: true,
    launcherPath,
...
function createWslRunner(initialFile: string | null = null, pathIncludesLocalBin = true) {
  const commandPath = '/home/alice/.local/bin/korca-ide'
  const bridgePath = '/home/alice/.local/share/korca/korca-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      const launcher =
        command.match(/cat > "\$command_tmp" <<'KORCA_WSL_CLI'\n([\s\S]*)\nKORCA_WSL_CLI/)?.[1] ?? ''
      const bridge =
        command.match(
    ...
    if (command.includes('rm -f')) {
      if (
        ...
        !files.get(bridgePath)?.includes('# Korca managed WSL CLI PowerShell bridge')
      ) {
        throw new Error('__KORCA_CONFLICT__')
      }
    ...
    if (command.includes('cat ')) {
      if (command.includes(commandPath)) {
        return files.get(commandPath) ?? '__KORCA_MISSING__'
      }
      if (command.includes(bridgePath)) {
        return files.get(bridgePath) ?? '__KORCA_MISSING__'
      }
    }
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows Korca launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/korca-ide'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\resources\\bin\\korca.cmd',
        '/home/alice/.local/share/korca/korca-wsl-bridge.ps1'
      )
    )
    ...
    expect(installCommand).toContain("legacy_command_path='/home/alice/.local/bin/korca'")
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/korca-ide')).toBe(
      '/home/alice/.local/share/korca/korca-wsl-bridge.ps1'
    )
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/korca')).toBe(
      '/home/alice/.local/share/korca/korca-wsl-bridge.ps1'
    )
  })
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Korca\\korca.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\Korca\\korca.cmd',
      '/home/alice/.local/share/korca/korca-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'Korca WSL CLI requires Windows interop and could not find powershell.exe.'
    )
    expect(launcher).toContain('"$KORCA_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File')
    expect(launcher).toContain('"$KORCA_WIN_LAUNCHER" "$@"')
    expect(launcher).not.toContain('-Command')
    expect(bridge).toContain('[Parameter(ValueFromRemainingArguments=$true)]')
    expect(bridge).toContain('& $KorcaLauncher @ForwardArgs')
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\bin\\korca.cmd',
      '/home/alice/.local/share/korca/korca-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\bin\\korca.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\resources\\bin\\korca.cmd'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Korca\\resources\\bin\\korca.cmd'
    })
  })
  ...
  it('refuses to remove an old managed launcher when the bridge path is user-owned', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Korca\\korca.cmd') },
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/korca/korca-wsl-bridge.ps1')) {
          return 'user bridge'
        }
        if (command.includes('rm -f')) {
          throw new Error('__KORCA_CONFLICT__')
    ...
    await expect(installer.remove()).rejects.toThrow('__KORCA_CONFLICT__')
  })
})
