const MANAGED_MARKER = '# Korca managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# Korca managed WSL CLI PowerShell bridge'

export function buildWslLauncher(
  windowsLauncherPath: string,
  bridgePath = '${XDG_DATA_HOME:-$HOME/.local/share}/korca/korca-wsl-bridge.ps1'
): string {
  const encodedTarget = Buffer.from(windowsLauncherPath, 'utf8').toString('base64')
  return `#!/usr/bin/env bash
set -euo pipefail
${MANAGED_MARKER}
# KORCA_WIN_LAUNCHER_B64=${encodedTarget}
KORCA_WIN_LAUNCHER=${quoteShell(windowsLauncherPath)}
KORCA_BRIDGE_PS1=${quoteShell(bridgePath)}
if command -v powershell.exe >/dev/null 2>&1; then
  KORCA_POWERSHELL=powershell.exe
elif [ -x /mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe ]; then
  KORCA_POWERSHELL=/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe
else
  echo "Korca WSL CLI requires Windows interop and could not find powershell.exe." >&2
  exit 1
fi
KORCA_BRIDGE_PS1_WIN=$(wslpath -w "$KORCA_BRIDGE_PS1")
exec "$KORCA_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File "$KORCA_BRIDGE_PS1_WIN" "$KORCA_WIN_LAUNCHER" "$@"
`
}
...
export function getBridgePathFromCommandPath(commandPath: string): string {
  ...
  // share one WSL bridge under ~/.local/share/korca.
  return `${commandPath.replace(/\/\.local\/bin\/(?:korca|korca-ide)$/, '/.local/share/korca')}/korca-wsl-bridge.ps1`
}
...
export function parseManagedLauncherTarget(content: string): string | null {
  const encoded = content.match(/^# KORCA_WIN_LAUNCHER_B64=([A-Za-z0-9+/=]+)$/m)?.[1]
  if (encoded) {
    try {
  ...
  const legacyTarget = content.match(/^KORCA_WIN_LAUNCHER='((?:[^']|'"'"')*)'$/m)?.[1]
