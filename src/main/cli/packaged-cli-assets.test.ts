...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/korca-ide', import.meta.url)
...
describe('packaged CLI assets', () => {
  ...
  itRunsUnixShell(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'korca-linux-cli-'))
      try {
        const appDir = join(root, 'Korca')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'korca-ide')
        const electronPath = join(appDir, 'korca-ide')
...
`,
        ...
        const commandPath = join(commandDir, 'korca-ide')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'korca'), { recursive: true })
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    const root = await mkdtemp(join(tmpdir(), 'korca-appimage-cli-'))
    try {
      const appDir = join(root, 'Korca.AppDir')
      const cliDir = join(appDir, 'resources', 'app.asar.unpacked', 'out', 'cli')
      const cliPath = join(cliDir, 'index.js')
      const appImagePath = join(root, "Korca's AppImage.AppImage")
      const commandPath = join(root, 'korca-ide')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
  console.log(JSON.stringify({
    ...
    korcaNodeOptions: process.env.KORCA_NODE_OPTIONS ?? null,
    nodeReplExternalModule: process.env.NODE_REPL_EXTERNAL_MODULE ?? null,
    korcaNodeReplExternalModule: process.env.KORCA_NODE_REPL_EXTERNAL_MODULE ?? null
  }))
}
`,
        'utf8'
...
`,
      ...
      const payload = JSON.parse(result.stdout) as {
        ...
        korcaNodeOptions: string | null
        nodeReplExternalModule: string | null
        korcaNodeReplExternalModule: string | null
      }
      ...
      expect(payload.korcaNodeOptions).toBe('--trace-warnings')
      expect(payload.nodeReplExternalModule).toBeNull()
      expect(payload.korcaNodeReplExternalModule).toBe('external-loader')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
