...
describe('handshake framing', () => {
  it('round-trips an korca-relay-handshake envelope through the existing framing', () => {
    ...
    expect(msg).toEqual({ type: 'korca-relay-handshake', version: '0.1.0+deadbeef' })
  })

  it('round-trips an korca-relay-handshake-ok reply', () => {
    const sent = encodeHandshakeFrame({
    ...
    decoder.feed(sent)
    const msg = parseHandshakeMessage(frames[0].payload)
    expect(msg).toEqual({ type: 'korca-relay-handshake-ok', version: '0.1.0+deadbeef' })
  })

  it('round-trips an korca-relay-handshake-mismatch reply', () => {
    const sent = encodeHandshakeFrame({
  ...
  it('rejects payloads with unknown type', () => {
    const bogus = Buffer.from(JSON.stringify({ type: 'korca-something-else', version: 'x' }))
