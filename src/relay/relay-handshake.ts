// Wire-level handshake helpers for the Korca relay.
//
...
export function setupDaemonHandshake(sock: Socket, cb: DaemonHandshakeCallbacks): void {
  ...
  ;(sock as Socket & { __korcaOnHandshake?: typeof onHandshakeData }).__korcaOnHandshake =
    onHandshakeData
}

export function detachHandshakeListener(sock: Socket): void {
  const tagged = sock as Socket & { __korcaOnHandshake?: (chunk: Buffer) => void }
  if (tagged.__korcaOnHandshake) {
    sock.removeListener('data', tagged.__korcaOnHandshake)
    delete tagged.__korcaOnHandshake
  }
}
...
function handleDaemonHandshakeFrame(
  ...
  if (msg.type !== 'korca-relay-handshake') {
    process.stderr.write(
  ...
  if (msg.version !== launchVersion) {
    ...
    try {
      sock.write(
        encodeHandshakeFrame({
          type: 'korca-relay-handshake-mismatch',
  ...
  sock.write(encodeHandshakeFrame({ type: 'korca-relay-handshake-ok', version: launchVersion }))
  return true
}
...
export function runConnectHandshake(
  ...
  const decoder: FrameDecoder = new FrameDecoder(
    (frame: DecodedFrame) => {
      ...
      if (msg.type === 'korca-relay-handshake-ok') {
      ...
      if (msg.type === 'korca-relay-handshake-mismatch') {
  ...
  sock.write(encodeHandshakeFrame({ type: 'korca-relay-handshake', version: myVersion }))
}
