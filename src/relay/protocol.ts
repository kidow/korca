...
export const RELAY_SENTINEL = `KORCA-RELAY v${RELAY_VERSION} READY\n`
...
export type HandshakeMessage =
  | { type: 'korca-relay-handshake'; version: string }
  | { type: 'korca-relay-handshake-ok'; version: string }
  | { type: 'korca-relay-handshake-mismatch'; expected: string; got: string }
...
export function parseHandshakeMessage(payload: Buffer): HandshakeMessage {
  ...
  if (
    t !== 'korca-relay-handshake' &&
    t !== 'korca-relay-handshake-ok' &&
    t !== 'korca-relay-handshake-mismatch'
  ) {
