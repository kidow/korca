...
type ManagedPty = {
  ...
  /** Renderer-supplied paneKey from spawn env (KORCA_PANE_KEY). Captured so
   *  external observers (the relay-hook-server cache) can evict per-pane
   *  state when this PTY exits. Symmetric with Korca's local pty.ts. */
  paneKey?: string
  tabId?: string
  worktreeId?: string
}
...
export class PtyHandler {
  ...
  // server's KORCA_AGENT_HOOK_* coords). Run on every spawn so every PTY
   ...
   *  relay-hook server to inject KORCA_AGENT_HOOK_PORT/TOKEN/ENV/VERSION/
   ...
   *  see the fresh KORCA_AGENT_HOOK_* coords just like freshly-spawned ones,
  ...
  private async spawn(
    ...
    // Why: server-side augmenter values (KORCA_AGENT_HOOK_* and plugin overlay
    ...
    const paneKey = typeof env?.KORCA_PANE_KEY === 'string' ? env.KORCA_PANE_KEY : undefined
    ...
    // separate ptyId→paneKey map. KORCA_PANE_KEY is shaped `${tabId}:${paneId}`
    // and is bounded by the renderer; the relay treats it as opaque.
    const tabId = typeof env?.KORCA_TAB_ID === 'string' ? env.KORCA_TAB_ID : undefined
    const worktreeId = typeof env?.KORCA_WORKTREE_ID === 'string' ? env.KORCA_WORKTREE_ID : undefined
  ...
  private async revive(params: Record<string, unknown>): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (entry.paneKey) {
        revivedEnv.KORCA_PANE_KEY = entry.paneKey
      }
      if (entry.tabId) {
        revivedEnv.KORCA_TAB_ID = entry.tabId
      }
      if (entry.worktreeId) {
        revivedEnv.KORCA_WORKTREE_ID = entry.worktreeId
      }
      const shell = resolveDefaultShell()
