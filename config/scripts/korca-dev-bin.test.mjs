...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'korca-dev.mjs')

describe('korca-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['korca-dev']).toBe('./config/scripts/korca-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8').startsWith('#!/usr/bin/env node\n')).toBe(true)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'korca-dev-bin-'))
    ...
    writeFileSync(
      ...
      [
        ...
        '  userDataPath: process.env.KORCA_USER_DATA_PATH,',
        '  appExecutable: process.env.KORCA_APP_EXECUTABLE',
        '}));'
      ].join('\n'),
      'utf8'
    )
    ...
    execFileSync(process.execPath, [wrapperPath, '--help'], {
      env: {
        ...process.env,
        KORCA_DEV_CLI_ENTRY_PATH: cliEntry,
        KORCA_DEV_USER_DATA_PATH: path.join(root, 'user-data'),
        KORCA_APP_EXECUTABLE: path.join(root, 'Electron')
      },
      stdio: 'ignore'
    })
