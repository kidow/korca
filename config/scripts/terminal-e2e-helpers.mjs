#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Korca
 ...
 *
 * Usage:
 *   import { KorcaTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new KorcaTerminal(9444)       // CDP port
 *   await term.connect()
...
export class KorcaTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Korca */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('korca-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the Korca window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('korca-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Korca.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('korca-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new KorcaTerminal(port)
  console.log('Connecting to Korca on CDP port', port, '...')
  term.connect()
