...
async function installElectronPackageBinary() {
  ...
  const tempDir = mkdtempSync(resolve(tmpdir(), 'korca-electron-'))
...
function getExtractorCommand(zipPath, extractDir) {
  if (process.env.KORCA_ELECTRON_PACKAGE_EXTRACTOR) {
    return {
      file: process.execPath,
      args: [process.env.KORCA_ELECTRON_PACKAGE_EXTRACTOR, zipPath, extractDir],
      label: `node ${process.env.KORCA_ELECTRON_PACKAGE_EXTRACTOR}`
    }
  }

  if (osPlatform() === 'win32') {
    return {
      file: process.env.KORCA_POWERSHELL_BIN || 'powershell',
      args: [
        '-NoProfile',
        '-NonInteractive',
        '-ExecutionPolicy',
  ...
  return {
    file: process.env.KORCA_UNZIP_BIN || 'unzip',
