...
REM launcher resolves APP_DIR back to resources/ and `korca open` cannot find
REM Korca.exe on packaged Windows installs.
for %%I in ("%RESOURCES_DIR%\..") do set "APP_DIR=%%~fI"
set "ELECTRON=%APP_DIR%\Korca.exe"

if not exist "%ELECTRON%" (
  echo Unable to locate Korca.exe next to "%RESOURCES_DIR%" 1>&2
  exit /b 1
)

REM Why: Korca packages the CLI entrypoint outside app.asar so the public shell
