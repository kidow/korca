...
name: orchestration
description: >-
  Use for Korca agent-to-agent coordination: send/ask/reply between agent
  ...
  Use `korca-cli` instead for terminal control, shell commands, browser
  automation, worktree management, and reading or waiting on terminals.
---

# Korca Inter-Agent Orchestration

Use this skill when the task involves coordinating multiple coding agents through Korca's orchestration system. For basic terminal and worktree management, use the `korca-cli` skill instead.
...
Use `korca-cli` instead for ordinary terminal control, shell commands, browser automation, worktree management, or reading/waiting on terminals.

## Preconditions

- Korca must be running (`korca status --json` should return `runtime: true`).
- The `korca` CLI must be on PATH (`korca-ide` on Linux; installed via Settings > Browser > Enable Korca CLI).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- All `korca orchestration` commands are RPC calls to the running Korca runtime — they require an active Korca session.
...
When the handoff type is unclear, inspect `korca orchestration task-list --json`, `korca orchestration dispatch-show`, and `korca terminal list --json` for the task, dispatch, and handle ownership before sending lifecycle messages. If you still cannot tell whether the remote handle owns an active dispatch, ask the current owner instead of silently completing a task into an unrelated workstream.
...
```bash
korca orchestration send --to <handle|@group> --subject <text> [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
korca orchestration check [--terminal <handle>] [--unread] [--types <type,...>] [--inject] [--wait] [--timeout-ms <n>] [--json]
korca orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
korca orchestration inbox [--limit <n>] [--json]
```

Why: `--from` auto-resolves via the `KORCA_TERMINAL_HANDLE` environment variable injected into every Korca-managed terminal. Omit it unless impersonating another terminal.
...
```bash
korca orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
korca orchestration task-list [--status <status>] [--ready] [--json]
korca orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
```
...
```bash
korca orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
korca orchestration dispatch-show --task <task_id> [--json]
```

Why: `--inject` sends a preamble that teaches the agent how to use `korca orchestration send --type worker_done` to report completion. All agents have `korca` (or `korca-ide` on Linux) on PATH and can execute shell commands. The preamble maximizes structured feedback but the system works without it (coordinator falls back to idle detection + output reading).
...
```bash
korca orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
korca orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
korca orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
```bash
korca orchestration run --spec <text> [--from <handle>] [--poll-interval-ms <n>] [--max-concurrent <n>] [--worktree <selector>] [--json]
korca orchestration run-stop [--json]
```

Why: `run` returns immediately with a run ID. The coordinator loop runs in the background inside the Korca runtime. Query progress via `korca orchestration task-list`. Only one coordinator can run at a time.
...
```bash
korca orchestration reset [--all] [--tasks] [--messages] [--json]
```
...
Coordinators need these terminal commands to spawn agents, monitor progress, and read output. Full terminal documentation lives in the `korca-cli` skill — this is the subset required for orchestration workflows.

```bash
korca terminal list [--worktree <selector>] [--json]
korca terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
korca terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
korca terminal read [--terminal <handle>] [--json]
korca terminal send [--terminal <handle>] --text <text> [--enter] [--json]
korca terminal wait [--terminal <handle>] --for <exit|tui-idle> [--timeout-ms <n>] [--json]
korca terminal show --terminal <handle> [--json]
korca terminal stop [--terminal <handle>] [--json]
korca terminal close [--terminal <handle>] [--json]
```

Why: `--terminal` is optional for most commands. When omitted, Korca auto-resolves to the active terminal in the current worktree.

Why: `--command "claude"` launches Claude Code in the new terminal. In local Korca sessions, `--command "codex"` launches Codex through Korca's visible terminal path automatically so Codex does not start as a headless/background PTY. After creating a `--command` terminal, use `terminal wait --for tui-idle` to wait for the agent to boot before dispatching.
...
Why: terminal handles are runtime-scoped. If Korca restarts, handles go stale. Re-acquire with `terminal list`.
...
- Use `korca orchestration check` to read incoming messages from the coordinator or other agents. Messages are delivered automatically when you go idle, but you can also poll explicitly.
- Treat `korca orchestration` commands the same way you treat `git` or `npm` — they are CLI tools available in your shell.
- The coordinator uses `korca orchestration task-list --ready` as its external memory. Prefer querying orchestration state over tracking it in your context window.
...
- Terminal handles are ephemeral and runtime-scoped. If Korca restarts mid-workflow, all handles go stale. Re-acquire them with `terminal list` before continuing.
...
korca terminal create --worktree active --title "worker-1" --command "claude" --json
# → handle: term_abc123

# 2. Wait for Claude Code to boot (tui-idle fires when the prompt appears)
korca terminal wait --terminal term_abc123 --for tui-idle --timeout-ms 60000 --json

# 3. Create and dispatch a task with preamble injection
korca orchestration task-create --spec "Fix the login button CSS" --json
# → id: task_def456
korca orchestration dispatch --task task_def456 --to term_abc123 --inject --json

# 4. Block until the worker reports back (no sleep loops needed)
korca orchestration check --wait --types worker_done,escalation --timeout-ms 300000 --json
# → returns immediately when worker sends worker_done

# 5. If --wait timed out with no messages, fall back to idle detection
korca terminal wait --terminal term_abc123 --for tui-idle --timeout-ms 60000 --json
korca terminal read --terminal term_abc123 --json
```
