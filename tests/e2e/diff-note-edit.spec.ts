...
test.describe('Diff note edit', () => {
  ...
  test('editing a saved inline note updates the open diff card', async ({ korcaPage }) => {
    ...
    const { relativePath } = await korcaPage.evaluate(async (wId) => {
    ...
    const addResult = await korcaPage.evaluate(
      async ({ wId, rel, body }) => {
        const store = window.__store
        if (!store) {
    ...
    const card = korcaPage.locator('.korca-diff-comment-card').first()
    await expect(card, 'seeded inline note did not render').toBeVisible({ timeout: 15_000 })
    await expect(card.locator('.korca-diff-comment-body')).toHaveText(seededBody)

    await card.getByTitle('Edit note').click()

    const textarea = card.locator('.korca-diff-comment-popover-textarea')
    await expect(textarea).toBeVisible()
    await expect(textarea).toHaveValue(seededBody)

    const saveButton = card
      .locator('.korca-diff-comment-popover-footer button')
      .filter({ hasText: 'Save' })
    ...
    const updatedCard = korcaPage
      .locator('.korca-diff-comment-card')
      .filter({ has: korcaPage.locator('.korca-diff-comment-body', { hasText: editedBody }) })
      .first()
    await expect(updatedCard, 'inline card did not update in the open diff').toBeVisible()
    await expect(updatedCard.locator('.korca-diff-comment-body')).toHaveText(editedBody)
    await expect(updatedCard.locator('.korca-diff-comment-body')).not.toHaveText(seededBody)
