...
test.describe('Worktree Lifecycle', () => {
  ...
  test.afterEach(async ({ korcaPage }) => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    const originalWorktreeId = await waitForActiveWorktree(korcaPage)

    createdWorktreeId = await createIsolatedWorktree(korcaPage)
    const newWorktreeId = createdWorktreeId
    await switchToWorktree(korcaPage, newWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), { timeout: 10_000 })
      .toBe(newWorktreeId)
    await ensureTerminalVisible(korcaPage)

    // Seed one of each surface on the new worktree so removeWorktree has to
    // clean up all three in a single atomic set().
    await korcaPage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await openFileExplorer(korcaPage)
    await clickFileInExplorer(korcaPage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(korcaPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getBrowserTabs(korcaPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getOpenFiles(korcaPage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    // is protecting.
    await switchToWorktree(korcaPage, originalWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), { timeout: 10_000 })
      .toBe(originalWorktreeId)

    const result = await removeWorktreeViaStore(korcaPage, newWorktreeId)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(korcaPage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(korcaPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(korcaPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)

    const allIds = await getAllWorktreeIds(korcaPage)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await korcaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    await switchToWorktree(korcaPage, otherWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), { timeout: 10_000 })
    ...
    const afterRoundTrip = await korcaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
   ...
   */
  test('terminal tabs stay scoped to the worktree that created them', async ({ korcaPage }) => {
    const allIds = await getAllWorktreeIds(korcaPage)
    expect(
      allIds.length,
      'fixture should provide primary + e2e-secondary worktrees'
    ...
    const worktreeA = await waitForActiveWorktree(korcaPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(korcaPage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(korcaPage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(korcaPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(korcaPage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(korcaPage, worktreeA)
    const tabsB = await getWorktreeTabs(korcaPage, worktreeB)
