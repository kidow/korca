...
test.describe('Worktree Recent Sort', () => {
  ...
  function createFolderFixture(): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'korca-e2e-folder-'))
  ...
  test.afterEach(() => {
    ...
    // matches the mkdtempSync/rmSync pairing used in helpers/korca-app.ts
    // and helpers/korca-restart.ts.
  ...
  test('stamps lastActivityAt on a newly-added folder repo so it sorts to the top of Recent', async ({
    ...
    const repoId = await addFolderRepo(korcaPage, folderPath)
    const lastActivityAt = await readFolderWorktreeLastActivity(korcaPage, repoId)
  ...
  test('leaves lastActivityAt stable across repeated list refreshes', async ({ korcaPage }) => {
    ...
    const first = await readFolderWorktreeLastActivity(korcaPage, repoId)

    await korcaPage.evaluate(async (id) => {
      await window.__store?.getState().fetchWorktrees(id)
      await window.__store?.getState().fetchWorktrees(id)
    }, repoId)

    const second = await readFolderWorktreeLastActivity(korcaPage, repoId)
