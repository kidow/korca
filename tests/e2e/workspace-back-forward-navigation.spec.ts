...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ korcaPage }) => {
    await expect(await getBackButton(korcaPage)).toBeVisible()
    await expect(await getForwardButton(korcaPage)).toBeVisible()
    ...
    await expect(await getBackButton(korcaPage)).toHaveCount(0)
    await expect(await getForwardButton(korcaPage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({ korcaPage }) => {
    ...
    const activeId = await getActiveWorktreeId(korcaPage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(korcaPage)
    await seedVisit(korcaPage, activeId!)

    const back = await getBackButton(korcaPage)
    const forward = await getForwardButton(korcaPage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ korcaPage }) => {
    const worktreeIds = await getAllWorktreeIds(korcaPage)
    ...
    await resetNavHistory(korcaPage)
    await seedVisit(korcaPage, primaryId)
    await seedVisit(korcaPage, secondaryId)
    ...
    const primaryRow = korcaPage.locator(
      `[id="worktree-list-option-${encodeURIComponent(primaryId)}"]`
    )
    const secondaryRow = korcaPage.locator(
      `[id="worktree-list-option-${encodeURIComponent(secondaryId)}"]`
    )

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ korcaPage }) => {
    const activeId = await getActiveWorktreeId(korcaPage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(korcaPage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(korcaPage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ korcaPage }) => {
    const worktreeIds = await getAllWorktreeIds(korcaPage)
    ...
    await seedVisit(korcaPage, primaryId)
    await seedVisit(korcaPage, secondaryId)
    await (await getBackButton(korcaPage)).click()
    await expect.poll(() => getActiveWorktreeId(korcaPage)).toBe(primaryId)

    // Forward button is live — a forward entry exists.
    await expect(await getForwardButton(korcaPage)).toBeEnabled()
    ...
    await expect(await getForwardButton(korcaPage)).toBeDisabled()
  })

  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({ korcaPage }) => {
    const worktreeIds = await getAllWorktreeIds(korcaPage)
    ...
    await korcaPage.keyboard.press(`${mod}+Alt+ArrowLeft`)
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), {
    ...
    await korcaPage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(korcaPage), {
  ...
  test('shortcut is a no-op in settings view', async ({ korcaPage }) => {
    const worktreeIds = await getAllWorktreeIds(korcaPage)
    ...
    await expect
      .poll(async () => korcaPage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(korcaPage)
    await korcaPage.evaluate(() => document.body.focus())
    await korcaPage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await korcaPage.waitForTimeout(150)
    expect(await getActiveWorktreeId(korcaPage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(korcaPage)
    expect(snapshot.index).toBe(1)
  })
})
