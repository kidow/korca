...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const originalTitle = await getActiveTabTitle(korcaPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    const tabLocator = tabLocatorByTitle(korcaPage, originalTitle)
    await tabLocator.dblclick()

    const renameInput = korcaPage.getByRole('textbox', {
    ...
    await expect
      .poll(async () => getActiveCustomTitle(korcaPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(korcaPage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await tabLocatorByTitle(korcaPage, originalTitle).click({ button: 'right' })
    await korcaPage.getByRole('menuitem', { name: 'Change Title', exact: true }).click()
    ...
    await korcaPage.keyboard.type('Context Menu Title')
    await renameInput.press('Enter')

    await expect
      .poll(async () => getActiveCustomTitle(korcaPage, worktreeId), { timeout: 3_000 })
      .toBe('Context Menu Title')
    await expect(tabLocatorByTitle(korcaPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    const originalTitle = await getActiveTabTitle(korcaPage, worktreeId)
    ...
    await expect(tabLocatorByTitle(korcaPage, originalTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(korcaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('renaming to an empty string resets the tab to its default title', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    ...
    const defaultTitle = await getActiveTabTitle(korcaPage, worktreeId)
    ...
    const tabLocator = tabLocatorByTitle(korcaPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(korcaPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(korcaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(korcaPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(korcaPage, worktreeId)
    const activeId = await getActiveTabId(korcaPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(korcaPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(korcaPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(korcaPage, 'Committed By Blur')).toBeVisible()
    expect(
      await korcaPage.evaluate(
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    const targetTabId = await getActiveTabId(korcaPage)
    ...
    const tabLocator = tabLocatorByTitle(korcaPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    const tabsBefore = (await getWorktreeTabs(korcaPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(korcaPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(korcaPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(korcaPage, worktreeId)).length).toBe(tabsBefore)
  })
})
