...
test.describe('Terminal Panes', () => {
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane right', async ({ korcaPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(korcaPage)

    await splitActiveTerminalPane(korcaPage, 'vertical')
    await waitForPaneCount(korcaPage, paneCountBefore + 1)

    const paneCountAfter = await countVisibleTerminalPanes(korcaPage)
   ...
   * - terminal panes can be split
   */
  test('can split terminal pane down', async ({ korcaPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(korcaPage)

    await splitActiveTerminalPane(korcaPage, 'horizontal')
    await waitForPaneCount(korcaPage, paneCountBefore + 1)
  ...
  test('split panes persist PTY bindings by stable UUID leaf id', async ({ korcaPage }) => {
    const paneCountBefore = await countVisibleTerminalPanes(korcaPage)

    await splitActiveTerminalPane(korcaPage, 'vertical')
    await waitForPaneCount(korcaPage, paneCountBefore + 1)

    const snapshot = await waitForPaneIdentitySnapshot(korcaPage, paneCountBefore + 1)
  ...
  test('terminal process receives KORCA_PANE_KEY with the active UUID leaf id', async ({
    korcaPage
  }) => {
    const snapshot = await waitForPaneIdentitySnapshot(korcaPage, 1)
    ...
    const ptyId = await discoverActivePtyId(korcaPage)
    const marker = `KORCA_PANE_KEY_E2E_${Date.now()}`

    await execInTerminal(korcaPage, ptyId, `printf '${marker}=%s\\n' "$KORCA_PANE_KEY"`)
    await waitForTerminalOutput(korcaPage, `${marker}=${expectedPaneKey}`)

    expect(activeLeafId).toMatch(UUID_RE)
  })

  test('terminal context menu copies the stable pane ID', async ({ korcaPage }) => {
    const snapshot = await waitForPaneIdentitySnapshot(korcaPage, 1)
    ...
    await openTerminalContextMenu(korcaPage)
    await korcaPage.getByText('Copy Pane ID', { exact: true }).click()

    await expect
      .poll(() => korcaPage.evaluate(() => window.api.ui.readClipboardText()), { timeout: 3_000 })
      .toBe(expectedPaneKey)
    await expect(korcaPage.getByText('Pane ID copied', { exact: true })).toBeVisible()
    expect(leafId).toMatch(UUID_RE)
  })

  test('first Set Title from terminal context menu stays open for typing', async ({ korcaPage }) => {
    const title = `First menu title ${Date.now()}`

    await openTerminalContextMenu(korcaPage)
    await korcaPage.getByText('Set Title…', { exact: true }).click()

    const titleInput = korcaPage.locator('.pane-title-input').first()
    await expect(titleInput).toBeVisible()
    ...
    await expect(titleInput).toHaveCount(0)
    await expect(korcaPage.locator('.pane-title-text', { hasText: title })).toHaveCount(1)
  })

  test('Set Title input stays open when clicked in a split terminal', async ({ korcaPage }) => {
    await splitActiveTerminalPane(korcaPage, 'vertical')
    await waitForPaneCount(korcaPage, 2)
    await splitActiveTerminalPane(korcaPage, 'horizontal')
    await waitForPaneCount(korcaPage, 3)
  ...
  test('Set Title survives an early blur during first focus handoff', async ({ korcaPage }) => {
    await openTerminalContextMenu(korcaPage)
  ...
  test('Set Title still commits by blur after focus settles', async ({ korcaPage }) => {
  ...
  test('Set Title stays pane-local during agent title churn', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    const tabId = (await getActiveTabId(korcaPage))!
    ...
    await setPaneTitleFromTerminalMenu(korcaPage, paneTitle)
    await expect(korcaPage.locator('.pane-title-text', { hasText: paneTitle })).toBeVisible()
    await expectTabCustomTitle(korcaPage, worktreeId, tabId, null)

    await korcaPage.getByRole('button', { name: `Edit pane title: ${paneTitle}` }).focus()
    await korcaPage.keyboard.press('Enter')
    const paneTitleInput = korcaPage.getByRole('textbox', { name: 'Pane title' })
    ...
    // Why: active agents continuously write OSC titles. Set Title is Korca's
    // pane-local overlay and must remain visible while the tab runtime title
    // continues to follow the active PTY.
    await expect(korcaPage.locator('.pane-title-text', { hasText: paneTitle })).toBeVisible()
    await expect(
      korcaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"]`)
    ).toHaveAttribute('data-tab-title', runtimeTitle)
    await expectTabCustomTitle(korcaPage, worktreeId, tabId, null)

    await setPaneTitleFromTerminalMenu(korcaPage, '')
    await expect(korcaPage.locator('.pane-title-text', { hasText: paneTitle })).toBeHidden()
    await expectSavedLayoutNotToContainTitle(korcaPage, tabId, paneTitle)

    await setPaneTitleFromTerminalMenu(korcaPage, removeButtonTitle)
    await korcaPage.locator('.pane-title-bar', { hasText: removeButtonTitle }).hover()
    await korcaPage.getByRole('button', { name: `Remove pane title: ${removeButtonTitle}` }).click()
    await expect(korcaPage.locator('.pane-title-text', { hasText: removeButtonTitle })).toBeHidden()
    await expectSavedLayoutNotToContainTitle(korcaPage, tabId, removeButtonTitle)

    await setPaneTitleFromTerminalMenu(korcaPage, splitTitle)
    await expectTabCustomTitle(korcaPage, worktreeId, tabId, null)

    await splitActiveTerminalPane(korcaPage, 'vertical')
    await waitForPaneCount(korcaPage, 2)
    await expect(korcaPage.locator('.pane-title-text', { hasText: splitTitle })).toBeVisible()
  ...
  test('closing a split pane prunes its leaf-keyed PTY binding without remapping siblings', async ({
    ...
    const beforeClose = await waitForPaneIdentitySnapshot(korcaPage, 3)
    ...
    await closeActiveTerminalPane(korcaPage)
    await waitForPaneCount(korcaPage, 2)

    const afterClose = await waitForPaneIdentitySnapshot(korcaPage, 2)
  ...
  test('closing and remaking right/down splits keeps surviving leaf-keyed bindings stable', async ({
    ...
    const afterRemake = await waitForPaneIdentitySnapshot(korcaPage, 3)
  ...
  test('moving panes through the drag-drop handler preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeMove = await waitForPaneIdentitySnapshot(korcaPage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(korcaPage)
    ...
    await moveTerminalPaneByLeafId(korcaPage, source.leafId, target.leafId, 'left')

    await expect
      .poll(async () => readTerminalPaneDomLeafOrder(korcaPage), {
        timeout: 10_000,
        message: 'Pane drag-drop move did not update DOM order'
      })
      .not.toEqual(beforeOrder)

    const afterMove = await waitForPaneIdentitySnapshot(korcaPage, 3)
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when switching tabs and back', async ({ korcaPage }) => {
    ...
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    await pressShortcut(korcaPage, 't')

    // Wait for the new tab to appear
    await expect
      .poll(async () => (await getWorktreeTabs(korcaPage, worktreeId)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Verify we're still on a terminal tab
    const activeType = await getActiveTabType(korcaPage)
    expect(activeType).toBe('terminal')

    // Switch back to the previous tab with Cmd/Ctrl+Shift+[
    await pressShortcut(korcaPage, 'BracketLeft', { shift: true })

    // Verify the marker is still present
    await expect
      .poll(async () => (await getTerminalContent(korcaPage)).includes(marker), { timeout: 5_000 })
      .toBe(true)

    // Clean up the extra tab
    await pressShortcut(korcaPage, 'BracketRight', { shift: true })
    await pressShortcut(korcaPage, 'w')
  })
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when splitting and closing a pane', async ({ korcaPage }) => {
    // Write a unique marker to the current terminal
    const ptyId = await discoverActivePtyId(korcaPage)
    ...
    const panesBefore = await countVisibleTerminalPanes(korcaPage)

    // Split the terminal right
    await splitActiveTerminalPane(korcaPage, 'vertical')
    await waitForPaneCount(korcaPage, panesBefore + 1)

    await focusLastTerminalPane(korcaPage)
    await closeActiveTerminalPane(korcaPage)
    await waitForPaneCount(korcaPage, panesBefore)
   ...
   * - terminal panes retain state when switching tabs and when you make / close a pane / switch worktrees
   */
  test('terminal pane retains content when switching worktrees and back', async ({ korcaPage }) => {
    ...
    await expect
      .poll(async () => (await getTerminalContent(korcaPage)).includes(marker), { timeout: 20_000 })
      .toBe(true)
  })
   ...
   * - resizing terminal panes works
   */
  test('shows a pane divider after splitting', async ({ korcaPage }) => {
    ...
    await expect(korcaPage.locator('.pane-divider.is-vertical').first()).toBeVisible({
      timeout: 3_000
    })
  })
   ...
   *   KORCA_E2E_HEADFUL=1 pnpm run test:e2e
   */
  test('@headful can resize terminal panes by real mouse drag', async ({ korcaPage }) => {
    ...
    const paneWidthsBefore = await korcaPage.evaluate(() => {
    ...
    const divider = korcaPage.locator('.pane-divider.is-vertical').first()
    ...
    await korcaPage.mouse.move(startX, startY)
    await korcaPage.mouse.down()
    await korcaPage.mouse.move(startX + 150, startY, { steps: 20 })
    await korcaPage.mouse.up()

    // Verify pane widths changed
    await expect
      .poll(
        async () => {
          const widthsAfter = await korcaPage.evaluate(() => {
            const xterms = document.querySelectorAll('.xterm')
            return Array.from(xterms)
              .filter((x) => (x as HTMLElement).offsetParent !== null)
              .map((x) => (x as HTMLElement).getBoundingClientRect().width)
  ...
  test('@headful resizing split panes forwards only the settled PTY size', async ({ korcaPage }) => {
    await splitActiveTerminalPane(korcaPage, 'vertical')
    const snapshot = await waitForPaneIdentitySnapshot(korcaPage, 2)
    ...
    await expect
      .poll(
        async () =>
          (await readVisiblePaneContents(korcaPage)).every((content) =>
    ...
    await korcaPage.mouse.move(startX, startY)
    await korcaPage.mouse.down()
    await korcaPage.mouse.move(startX - 350, startY, { steps: 40 })
    await korcaPage.mouse.move(startX + 250, startY, { steps: 40 })
    await korcaPage.mouse.up()
    await korcaPage.waitForTimeout(500)

    const paneContents = await readVisiblePaneContents(korcaPage)
  ...
  test('@headful dragging terminal panes around preserves leaf-keyed PTY bindings', async ({
    ...
    const beforeDrag = await waitForPaneIdentitySnapshot(korcaPage, 3)
    const beforeOrder = await readTerminalPaneDomLeafOrder(korcaPage)
    ...
    const sourceHandle = korcaPage.locator(
      `.pane[data-leaf-id="${source.leafId}"] .pane-drag-handle`
    )
    await expect(sourceHandle).toBeVisible({ timeout: 3_000 })
    const sourceBox = await sourceHandle.boundingBox()
    const targetBox = await korcaPage.locator(`.pane[data-leaf-id="${target.leafId}"]`).boundingBox()
    expect(sourceBox).not.toBeNull()
    expect(targetBox).not.toBeNull()

    await korcaPage.mouse.move(sourceBox!.x + sourceBox!.width / 2, sourceBox!.y + 4)
    await korcaPage.mouse.down()
    await korcaPage.mouse.move(targetBox!.x + 8, targetBox!.y + targetBox!.height / 2, {
      steps: 20
    })
    await korcaPage.mouse.up()
    ...
    const afterDrag = await waitForPaneIdentitySnapshot(korcaPage, 3)
   ...
   * - closing panes works
   */
  test('closing a split pane removes it and remaining pane fills space', async ({ korcaPage }) => {
    const panesBefore = await countVisibleTerminalPanes(korcaPage)
    ...
    const panesAfterSplit = await countVisibleTerminalPanes(korcaPage)
    expect(panesAfterSplit).toBeGreaterThanOrEqual(2)

    await closeActiveTerminalPane(korcaPage)
    await waitForPaneCount(korcaPage, panesAfterSplit - 1)

    // The remaining pane should fill the available space
    const paneWidth = await korcaPage.evaluate(() => {
      const xterms = document.querySelectorAll('.xterm')
