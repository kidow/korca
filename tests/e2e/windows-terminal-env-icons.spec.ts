...
test.describe('Windows terminal env and shell identity', () => {
  ...
  test('dev terminal preserves parent PATH so PATH commands resolve', async ({ korcaPage }) => {
    await waitForActiveTerminalManager(korcaPage)

    const ptyId = await waitForActivePanePtyId(korcaPage)
    const marker = `__KORCA_E2E_NODE_PATH_${Date.now()}__`

    // Why: before the dev PATH fallback, daemon-spawned PTYs could get PATH set
    // to only Korca's dev CLI bin. A real terminal command catches that failure.
    await execInTerminal(korcaPage, ptyId, `node -e "console.log('${marker}')"`)

    await waitForTerminalOutput(korcaPage, marker, 15_000)
  })

  test('Windows tab icons stay pinned to the shell used at tab creation', async ({ korcaPage }) => {
    test.skip(process.platform !== 'win32', 'Windows shell icons only render on Windows')

    const tabIds = await korcaPage.evaluate(() => {
    ...
    const tabSnapshot = await korcaPage.evaluate(({ wslTabId, cmdTabId }) => {
    ...
    const wslTab = korcaPage.locator(
      `[data-testid="sortable-tab"][data-tab-id="${tabIds.wslTabId}"]`
    )
    const cmdTab = korcaPage.locator(
