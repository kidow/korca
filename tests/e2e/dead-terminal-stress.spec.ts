...
test.describe('Dead Terminal Stress @headful', () => {
   ...
   */
  test('@headful setup-split with forced WebGL context loss recovers', async ({ korcaPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(korcaPage)
    await waitForActiveTerminalManager(korcaPage, 30_000)

    for (let i = 0; i < STRESS_ITERATIONS; i++) {
      const newId = await createAndActivateWorktreeWithSetup(korcaPage, `ctxloss-${i}`, 'vertical')
      ...
      const lostCount = await korcaPage.evaluate(() => {
      ...
      await waitForAllPanesToHaveContent(korcaPage, `ctxloss-${i} after context loss`)
   ...
   */
  test('@headful rapid worktree switching during setup-split lifecycle', async ({ korcaPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(korcaPage)
    await waitForActiveTerminalManager(korcaPage, 30_000)

    for (let i = 0; i < 3; i++) {
      const newId = await createAndActivateWorktreeWithSetup(korcaPage, `rapid-${i}`, 'vertical')
      createdWorktreeIds.push(newId)

      // Switch away during the ~200ms scheduleSplitScrollRestore window
      await korcaPage.waitForTimeout(50)
      await switchToWorktree(korcaPage, homeWorktreeId)
      await korcaPage.waitForTimeout(50)
      ...
      await waitForActiveTerminalManager(korcaPage, 30_000)
      await waitForPaneCount(korcaPage, 2, 15_000)
      await waitForAllPanesToHaveContent(korcaPage, `rapid-${i} after return`)
