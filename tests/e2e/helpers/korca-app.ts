/**
 * Shared Electron fixture for Korca E2E tests.
 *
...
import { getKorcaElectronLaunchArgs } from './electron-launch-args'
import { getE2ECompletedOnboardingProfile } from './e2e-completed-onboarding-profile'

type KorcaTestFixtures = {
...
type KorcaWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad KORCA_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const KORCA_E2E_SLOWMO_MS_RAW = process.env.KORCA_E2E_SLOWMO_MS
const KORCA_E2E_SLOWMO_MS = ((): number => {
  if (KORCA_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(KORCA_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[korca-e2e] KORCA_E2E_SLOWMO_MS="${KORCA_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: KORCA_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.KORCA_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.korcaHeadful === true
}

function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.KORCA_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
/**
 * Extended Playwright test with Korca-specific fixtures.
 *
 * `korcaPage` — the main Korca renderer window.
 *
 ...
 */
export const test = base.extend<KorcaTestFixtures, KorcaWorkerFixtures>({
  ...
  electronApp: async ({ dismissOnboarding }, provideFixture, testInfo) => {
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'korca-e2e-userdata-'))
    ...
    void _unused
    // Why: KORCA_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with KORCA_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = KORCA_E2E_SLOWMO_MS
    // Why: KORCA_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir = process.env.KORCA_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: getKorcaElectronLaunchArgs(mainPath, headful),
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. KORCA_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: KORCA_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: KORCA_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. KORCA_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        KORCA_E2E_USER_DATA_DIR: userDataDir,
        ...(process.env.KORCA_E2E_SSH_LOCALHOST === '1' && !cleanEnv.KORCA_RELAY_PATH
          ? { KORCA_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { KORCA_E2E_HEADFUL: '1' } : { KORCA_E2E_HEADLESS: '1' })
      }
    })
  ...
  korcaPage: async ({ sharedPage }, provideFixture) => {
