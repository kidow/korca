...
async function ensureSetupHookCommitted(page: TestPage): Promise<void> {
  ...
  writeFileSync(path.join(activeWorktreePath, 'korca.yaml'), SETUP_HOOK_CONTENT, 'utf-8')
  execFileSync('git', ['add', 'korca.yaml'], { cwd: activeWorktreePath, stdio: 'pipe' })

  try {
    execFileSync('git', ['diff', '--cached', '--quiet'], { cwd: activeWorktreePath, stdio: 'pipe' })
  } catch {
    // Why: worktree creation reads korca.yaml from the created worktree, so the
