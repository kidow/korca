 ...
 *
 * Why: the default `korcaPage` fixture creates a fresh `userDataDir` per test
...
export function createRestartSession(testInfo: TestInfo): RestartSession {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'korca-e2e-restart-'))
  ...
  const launch = async (): Promise<LaunchedKorca> => {
...
export async function attachRepoAndOpenTerminal(page: Page, repoPath: string): Promise<string> {
  ...
  const repoId = await page.evaluate(async (repoPath) => {
    ...
    // worktree is external to Korca's workspace root after the visibility rollout.
