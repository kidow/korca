...
test.describe('Tab visibility with closed sidebar', () => {
  ...
  test('first tab stays visible after Cmd+B collapses the sidebar', async ({ korcaPage }) => {
    ...
    await pressShortcut(korcaPage, 'b')
  ...
  test('sidebar toggle and Back button stay separated after sidebar collapse', async ({
    ...
    await korcaPage.reload({ waitUntil: 'domcontentloaded' })
    await korcaPage.waitForFunction(() => Boolean(window.__store), null, { timeout: 30_000 })
    await waitForSessionReady(korcaPage)
    await waitForActiveWorktree(korcaPage)
    await ensureTerminalVisible(korcaPage)
