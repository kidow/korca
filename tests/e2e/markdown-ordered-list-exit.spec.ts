...
test.describe('Markdown ordered-list exit regression', () => {
  ...
  for (const row of rows) {
    test(row.name, async ({ korcaPage }, testInfo) => {
      const context = await getActiveWorktreeContext(korcaPage)
      ...
      try {
        ...
        const activeFile = await openMarkdownFixture(korcaPage, context, filePath)
        const draftKey = activeFile.filePath

        await row.run(korcaPage, row.sentinel)

        await expectSentinelParagraphOutsideOrderedList(korcaPage, row.sentinel)
        await expectSerializedDraftOutsideOrderedList(korcaPage, draftKey, row.sentinel)
      } finally {
