...
test.describe('File Open & Markdown Preview', () => {
   ...
   * - you can open files (from the right sidebar)
   */
  test('opening the right sidebar shows file explorer', async ({ korcaPage }) => {
    await openFileExplorer(korcaPage)

    // Why: the load-bearing check is that `FileExplorer` actually mounted.
    // `data-korca-explorer-shell` is the stable marker the component renders
    ...
    // painted empty.
    await expect(korcaPage.locator('[data-korca-explorer-shell]')).toBeVisible({ timeout: 5_000 })
  })
   ...
   * - you can open files (from the right sidebar)
   */
  test('clicking a file in the file explorer opens it in an editor tab', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    await openFileExplorer(korcaPage)

    const filesBefore = await getOpenFiles(korcaPage, worktreeId)

    // Click a known non-directory file
    const clickedFile = await clickFileInExplorer(korcaPage, [
      'package.json',
      'tsconfig.json',
      '.gitignore',
      'README.md'
    ...
    await expect.poll(async () => getActiveTabType(korcaPage), { timeout: 5_000 }).toBe('editor')

    // There should be a new open file
    await expect
      .poll(async () => (await getOpenFiles(korcaPage, worktreeId)).length, { timeout: 5_000 })
    ...
    await expect(korcaPage.locator('.editor-header-path').first()).toContainText(clickedFile!, {
      timeout: 20_000
    })
  })
   ...
   * - you can open .md files and they show up as preview (from the right sidebar)
   */
  test('opening a .md file shows markdown content', async ({ korcaPage }) => {
    await openFileExplorer(korcaPage)
    const clickedFile = await clickFileInExplorer(korcaPage, ['README.md', 'CLAUDE.md'])
    expect(clickedFile).not.toBeNull()

    // Wait for the editor tab to become active
    await expect.poll(async () => getActiveTabType(korcaPage), { timeout: 5_000 }).toBe('editor')

    // The seeded README.md starts with `# Korca E2E Test Repo`, so the rich
    ...
    await expect(korcaPage.getByRole('heading', { name: expectedHeading, level: 1 })).toBeVisible({
      timeout: 25_000
    })
  })
   ...
   */
  test('editor tab retains state when switching to terminal and back', async ({ korcaPage }) => {
    const worktreeId = (await getActiveWorktreeId(korcaPage))!
    await openFileExplorer(korcaPage)
    ...
    const openFilesBefore = await getOpenFiles(korcaPage, worktreeId)
    expect(openFilesBefore.length).toBeGreaterThan(0)

    const editorFileId = openFilesBefore[0].id

    // Switch to a terminal tab
    await switchToTerminal(korcaPage, worktreeId)
    await expect.poll(async () => getActiveTabType(korcaPage), { timeout: 3_000 }).not.toBe('editor')

    // Switch back to the same editor tab
    await switchToEditor(korcaPage, editorFileId)
    await expect.poll(async () => getActiveTabType(korcaPage), { timeout: 3_000 }).toBe('editor')

    // The same files should still be open
    const openFilesAfter = await getOpenFiles(korcaPage, worktreeId)
