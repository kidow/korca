...
test.describe('Diff note delete', () => {
  ...
  test('clicking the trash button removes the saved note', async ({ korcaPage }) => {
    ...
    const deleteButton = korcaPage.getByTitle('Delete note').first()
    ...
    await expect(korcaPage.getByTitle('Delete note')).toHaveCount(0, {
      timeout: 5_000
    })
  })
})
