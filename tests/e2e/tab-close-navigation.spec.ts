 ...
 * - PR #677 (`return to Korca landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ korcaPage }) => {
    const worktreeId = await waitForActiveWorktree(korcaPage)

    const fileIds = await openSeededEditorTabs(korcaPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(korcaPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(korcaPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(korcaPage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(korcaPage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ korcaPage }) => {
    const worktreeId = await waitForActiveWorktree(korcaPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(korcaPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await korcaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(korcaPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(korcaPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ korcaPage }) => {
    const worktreeId = await waitForActiveWorktree(korcaPage)
    ...
    const editorIds = await openSeededEditorTabs(korcaPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(korcaPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(korcaPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    const surfaceCounts = await korcaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    await closeFile(korcaPage, editorIds[0])
