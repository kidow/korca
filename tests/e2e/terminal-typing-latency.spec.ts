...
test.describe('Terminal typing latency', () => {
  test('interactive prompt echoes typed keys without visible lag', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.korca-typing-benchmark-${runId}.mjs`)
    writeFileSync(scriptPath, interactivePromptScript(runId))
    let commandSent = false
    try {
      await sendToTerminal(korcaPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      commandSent = true
      await waitForTerminalOutput(korcaPage, `TYPING_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(korcaPage)
      ...
      for (const [index, char] of [...KEY_LATENCY_SAMPLES].entries()) {
        ...
        const start = performance.now()
        await korcaPage.keyboard.type(char)
        await waitForMarkerLatency(korcaPage, marker, MAX_WORST_KEY_LATENCY_MS)
      ...
      if (commandSent) {
        await sendToTerminal(korcaPage, ptyId, '\x03').catch(() => undefined)
      }
