...
React Native companion app for Korca. Monitor worktrees, view terminal output, and send commands from your phone.

Local development uses two processes:

- Korca desktop/Electron from the repo root. This hosts the mobile WebSocket RPC server on port `6768`.
...
## Pair With Desktop Korca

1. Open Korca desktop.
...
korca fill --element @e1 --value "ls" --json
korca screenshot --json
```
...
```bash
cd mobile
KORCA_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/test-subscribe.ts <deviceToken> <serverPublicKeyB64>
```
...
```bash
cd mobile
KORCA_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/repro-terminal-colors.ts \
...
Develop the mobile app without a running Korca desktop instance:
...
## Connecting to Real Korca

1. Start Korca desktop with WebSocket transport enabled
2. In Korca, go to Settings > Mobile and scan the QR code with this app
