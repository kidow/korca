...
type KorcaYamlTrustPrompt = WorkspaceCreateArgs & {
  repoId: string
  repoName: string
  scriptContent: string
  contentHash: string
...
type ProjectRepoNotInKorcaPrompt = {
  owner: string
  repo: string
  url: string | null
}
...
export default function MobileTasksScreen() {
  ...
  const [korcaYamlTrustPrompt, setKorcaYamlTrustPrompt] = useState<KorcaYamlTrustPrompt | null>(null)
  ...
  const [projectRepoNotInKorca, setProjectRepoNotInKorca] =
    useState<ProjectRepoNotInKorcaPrompt | null>(null)
  ...
  useEffect(() => {
    ...
    const hydrateTaskState = async (): Promise<void> => {
      ...
      if (!status.capabilities?.includes(MOBILE_TASKS_CAPABILITY)) {
        ...
        setMergeMethodProjectRow(null)
        resetWorkspaceCreateState()
        setError('Update Korca desktop to use Tasks on mobile.')
        setTaskStateHydrated(false)
        return
      }
      ...
      const uiState = isSuccess(uiResponse)
        ...
        : null
      setTrustedKorcaHooks(uiState?.trustedKorcaHooks ?? {})
    ...
    async (project: GitHubProjectRef, options: { viewNumber?: number } = {}): Promise<void> => {
      ...
      try {
        ...
        if (explicitView && explicitView.layout !== 'TABLE_LAYOUT') {
          throw new Error("Korca doesn't support this GitHub Project layout yet.")
        }
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          // Why: desktop prompts before running repo-owned korca.yaml hooks. Mobile
          ...
          setKorcaYamlTrustPrompt({
            ...
            previouslyApproved: wasSetupHookPreviouslyApproved(trustedKorcaHooks, targetRepo.id)
          })
          return
        }
  ...
  const createWorkspaceFromProjectRow = useCallback(
    async (row: GitHubProjectRow): Promise<void> => {
      ...
      if (!kind || !row.content.number || !row.content.url) {
        setError('Add the project item repository to Korca before creating a workspace.')
        return
      }
      if (!repo) {
        const slug = splitRepositorySlug(row.content.repository)
        setProjectRepoNotInKorca({
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!tasksSupported ? (
        tasksUnsupported ? (
          <View style={styles.centered}>
            <Text style={styles.emptyText}>Update Korca desktop</Text>
      ...
      <PickerModal
        ...
        onSelect={(viewId) => {
          ...
          if (view && view.layout !== 'TABLE_LAYOUT') {
            setGithubProjectError("Korca doesn't support this GitHub Project layout yet.")
            return
          }
      ...
      <BottomDrawer
        visible={taskUiReady && korcaYamlTrustPrompt != null}
        onClose={() => setKorcaYamlTrustPrompt(null)}
        zIndex={TASK_SECONDARY_DRAWER_Z_INDEX + 1}
      >
        {korcaYamlTrustPrompt ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>
                {korcaYamlTrustPrompt.previouslyApproved
                  ? `${korcaYamlTrustPrompt.repoName}'s setup script changed`
                  : `Run setup from ${korcaYamlTrustPrompt.repoName}?`}
              </Text>
              <Text style={styles.sheetSubtitle}>
                This repository's korca.yaml runs on your machine before the workspace starts. Only
            ...
            <View style={styles.setupPromptBox}>
              <View style={styles.detailSectionHeader}>
                <Text style={styles.detailSectionTitle}>
                  {korcaYamlTrustPrompt.previouslyApproved ? 'New setup script' : 'Setup script'}
                </Text>
              </View>
              <Text style={styles.setupPromptCommand}>{korcaYamlTrustPrompt.scriptContent}</Text>
            </View>

            <View style={styles.actionGroup}>
              <Pressable
                style={styles.actionRow}
                disabled={creatingKey === korcaYamlTrustPrompt.item.key}
                onPress={() =>
                  void (async () => {
                    try {
                      await persistSetupHookTrust(
                        korcaYamlTrustPrompt.repoId,
                        korcaYamlTrustPrompt.contentHash,
                        false
                      )
                      setKorcaYamlTrustPrompt(null)
                      await createWorkspace(
                        korcaYamlTrustPrompt.item,
                        korcaYamlTrustPrompt.repoIdOverride,
                        'run',
                        korcaYamlTrustPrompt.agentOverride,
                        korcaYamlTrustPrompt.workspaceNameOverride,
                        korcaYamlTrustPrompt.noteOverride,
                        korcaYamlTrustPrompt.baseBranchOverride,
                        korcaYamlTrustPrompt.branchNameOverride,
                        korcaYamlTrustPrompt.sparseCheckoutOverride,
                        korcaYamlTrustPrompt.contentHash
                      )
                    } catch (err) {
              ...
              <Pressable
                ...
                disabled={creatingKey === korcaYamlTrustPrompt.item.key}
                onPress={() => {
                  const prompt = korcaYamlTrustPrompt
                  setKorcaYamlTrustPrompt(null)
                  void createWorkspace(
                    prompt.item,
      ...
      <BottomDrawer
        visible={taskUiReady && projectRepoNotInKorca != null}
        onClose={() => {
          setProjectRepoNotInKorca(null)
        }}
      >
        {projectRepoNotInKorca ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Repository not in Korca</Text>
              <Text style={styles.sheetSubtitle}>
                {projectRepoNotInKorca.owner}/{projectRepoNotInKorca.repo} is not added to Korca. Add
                this repository from the desktop app, then refresh mobile Tasks.
              </Text>
            </View>

            <View style={styles.actionGroup}>
              {projectRepoNotInKorca.url ? (
                <Pressable
                  style={styles.actionRow}
                  onPress={() => {
                    if (projectRepoNotInKorca.url) {
                      void Linking.openURL(projectRepoNotInKorca.url)
                    }
                  }}
                >
              ...
              {projectRepoNotInKorca.url ? <View style={styles.actionSeparator} /> : null}
              <Pressable
                style={styles.actionRow}
                onPress={() =>
                  void copyTextToClipboard(
                    `project-repo:${projectRepoNotInKorca.owner}/${projectRepoNotInKorca.repo}`,
                    `${projectRepoNotInKorca.owner}/${projectRepoNotInKorca.repo}`
                  )
                }
              >
            ...
            <View style={styles.actionGroup}>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  {!projectRowHostedRepo ? (
                    <Text style={styles.emptyInlineText}>
                      Merge requires this repository in Korca.
                    </Text>
                  ) : null}
                </>
              ) : null}
