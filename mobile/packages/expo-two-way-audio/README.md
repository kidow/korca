...
Vendored Expo module for capturing and playing PCM audio data in the Korca mobile app (iOS and Android).
...
```
npm i @korca/expo-two-way-audio
```
...
1. Request permissions for recording audio
   ...
   ```JSX
   import {useMicrophonePermissions} from "@korca/expo-two-way-audio";
...
Some audio features of expo-two-way-audio like Acoustic Echo Cancelling, noise reduction or microphone modes (iOS) don't work on simulator. Run the Korca mobile app on a real device to test these features.
