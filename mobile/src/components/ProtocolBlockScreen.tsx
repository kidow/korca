...
const RELEASES_URL = 'https://github.com/stablyai/korca/releases'
const IOS_APP_STORE_URL = 'itms-apps://apps.apple.com/app/korca-ide/id6766130217'
...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const title = isMobileTooOld ? 'Update Korca Mobile' : 'Update Korca on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer Korca Mobile app. Update Korca Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current Korca Mobile app. Update Korca on your computer, then try this host again.'
  const recoveryNote =
