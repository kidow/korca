...
describe('extractGitHubIssueSourceError', () => {
  it('keeps the failing issue source slug with the repo that produced it', () => {
    expect(
      extractGitHubIssueSourceError(
        { id: 'repo-1', path: '/work/korca' },
        {
          sources: { issues: { owner: 'upstream', repo: 'korca' } },
      ...
      repoId: 'repo-1',
      repoPath: '/work/korca',
      source: { owner: 'upstream', repo: 'korca' },
  ...
  it('returns null when the envelope has no issue-side error', () => {
    expect(
      extractGitHubIssueSourceError(
        { id: 'repo-1', path: '/work/korca' },
        {
          sources: { issues: { owner: 'stablyai', repo: 'korca' } }
        }
      )
    ).toBeNull()
  })
...
describe('extractGitHubIssueSourceFallback', () => {
  it('reports the repo whose upstream issue source fell back to origin', () => {
    expect(
      extractGitHubIssueSourceFallback(
        { id: 'repo-1', path: '/work/korca', displayName: 'korca' },
        {
          issueSourceFellBack: true,
          sources: {
            issues: { owner: 'stablyai', repo: 'korca-fork' },
            prs: { owner: 'stablyai', repo: 'korca' }
          }
        }
      )
    ).toEqual({
      repoId: 'repo-1',
      repoPath: '/work/korca',
      repoLabel: 'stablyai/korca'
    })
  })

  it('uses the Korca repo display name when the PR source is unavailable', () => {
    expect(
      extractGitHubIssueSourceFallback(
        { id: 'repo-1', path: '/work/korca', displayName: 'korca' },
        {
